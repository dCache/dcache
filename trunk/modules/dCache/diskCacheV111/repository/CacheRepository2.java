// $Id: CacheRepository2.java,v 1.46 2007-10-15 18:53:08 behrmann Exp $
...
public class CacheRepository2 implements CacheRepository {
    ...
    private final Map      _pnfsids        = new HashMap() ;
    private final Map      _removedPnfsids = new HashMap() ;
    ...
    public class CacheEntry  implements CacheRepositoryEntry  {
       ...
       public String toString(){
          return _pnfsId.toString()+
                 ...
                 "("+getRestLock()+")"+
                 "["+_links+"]> "+
                 ((_state & (CACHED | PRECIOUS) ) > 0 ? _storageInfo.getFileSize() : getSize()) +
       ...
       private void _write2() throws CacheException {
           ...
           str = (_state & PRECIOUS)    > 0  ? "precious" :
                 ...
                 (_state & FROM_CLIENT) > 0  ? "receiving.client" :
    ...
    public void setSticky(boolean sticky, String owner, long validTill)
			throws CacheException {
		// owner and valid time are not supported by this CacheEntry implementation 
		setSticky(sticky);		
	}
	public List<StickyRecord> stickyRecords() {
		throw new UnsupportedOperationException("stickyRecords() not supported in this CacheEntry implementation");		
	}
    }
    //
    // -------------------------------------------------------------------------
