// $Id: EasyCopyCellModule.java,v 1.3 2006-09-14 13:24:14 patrick Exp $
//
...
public class EasyCopyCellModule implements  CommandTaskCell.CellCommandTaskable, java.io.Serializable {
   ...
   private Map          _inProgressMap  = null ;
...
//            IllegalStateException("Nothing in progress");
         ...
         list = new ArrayList( _inProgressMap.values() ) ; 
     }
     for( Iterator i =list.iterator() ; i.hasNext() ; ){
         RepositoryEntry entry = (RepositoryEntry)i.next() ;
   ...
   private class RepositoryStatistics {
      ...
      private Map storageClassMap = new HashMap() ;
      ...
      private void scan( RepositoryEntry entry , boolean withError ){
          ...
          long [] counter = (long [])storageClassMap.get(entry._info);
          ...
          errorCount ++ ;
          counter = (long [])errorMap.get( entry._message ) ;
      ...
      private String toTable(){
         ...
         if( errorMap.size() > 0 ){
            ...
            for( Iterator i = errorMap.entrySet().iterator() ; i.hasNext() ; ){
               Map.Entry   e = (Map.Entry)i.next() ;
               String   name = (String)e.getKey() ;
               long [] value = (long [])e.getValue() ;
   ...
   private class PoolInfoHandler {
      private class PoolInfo implements Comparable {
      ...
      private Map _pools = new HashMap() ;
      ...
      private void update( String poolName , long free , long removable ){
         PoolInfo info = (PoolInfo)_pools.get(poolName) ;
      ...
      private PoolInfo getBestPool(){
        TreeSet set = new TreeSet( _pools.values() ) ;
   ...
   private void replyForCopytoArrived( Object sendObj , Object obj ){
      ...
      if( sendObj instanceof PnfsGetStorageInfoMessage ){
          ...
          synchronized( _lock ){
              ...
              RepositoryEntry entry = (RepositoryEntry)_inProgressMap.remove(pnfsId);
              if( entry == null ){
