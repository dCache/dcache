...
public class HsmCleaner extends AbstractCleaner implements CellMessageReceiver, CellCommandListener,
    ...
    /**
     * Returns timeout in milliseconds for delete requests send to pools.
     */
    ...
    protected void onSuccess(URI uri) {
        try {
            _log.debug("HSM-ChimeraCleaner: remove entries from the trash-table. ilocation={}",
                  uri);
    ...
    public synchronized void messageArrived(PoolRemoveFilesFromHSMMessage msg) {
        ...
        if (!failures.isEmpty()) {
            _log.warn("Failed to delete {} files from HSM {}. Will try again later.",
                  failures.size(), hsm);
        }
    ...
    public String ac_hsm_set_MaxFilesPerRequest_$_1(Args args)
        ...
        return "Maximal number of concurrent requests to a single HSM is set to "
              + _hsmCleanerRequest;
    }
    ...
    public String ac_hsm_set_TimeOut_$_1(Args args) throws NumberFormatException, CommandException {
        ...
        return "Timeout for cleaning requests to HSM-pools is set to " + _hsmTimeout + " "
              + _hsmTimeoutUnit;
    }
