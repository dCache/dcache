...
public class AxisSrmFileSystem implements SrmFileSystem {
    ...
    @Nonnull
    @Override
    public TMetaDataPathDetail stat(URI surl)
    ...
    @Nonnull
    @Override
    public TMetaDataPathDetail[] stat(URI... surls)
          throws RemoteException, SRMException, InterruptedException {
    ...
    private TMetaDataPathDetail[] doBulkStat(URI[] surls)
          throws RemoteException, SRMException, InterruptedException {
        SrmLsResponse response = srm.srmLs(
              new SrmLsRequest(null, new ArrayOfAnyURI(surls), null, null, true, false, 0, 0,
                    surls.length));
        ...
        if (response.getReturnStatus().getStatusCode() != TStatusCode.SRM_REQUEST_QUEUED &&
            ...
            checkSuccess(response.getReturnStatus(), TStatusCode.SRM_SUCCESS,
                  TStatusCode.SRM_PARTIAL_SUCCESS, TStatusCode.SRM_FAILURE);
            ...
            do {
                ...
                status = srm.srmStatusOfLsRequest(
                      new SrmStatusOfLsRequestRequest(null, response.getRequestToken(), 0,
                            surls.length));
    ...
    @Nonnull
    @Override
    public TSURLPermissionReturn[] checkPermissions(URI... surls)
    ...
    public TPermissionReturn[] getPermissions(URI... surls) throws RemoteException, SRMException {
        ...
        TPermissionReturn[] permissionArray = response.getArrayOfPermissionReturns()
              .getPermissionArray();
        if (permissionArray == null || permissionArray.length == 0) {
            checkSuccess(response.getReturnStatus(), TStatusCode.SRM_SUCCESS,
                  TStatusCode.SRM_PARTIAL_SUCCESS);
        ...
        for (TPermissionReturn permission : permissionArray) {
            ...
            if (permission.getArrayOfUserPermissions().getUserPermissionArray() == null) {
                permission.getArrayOfUserPermissions()
                      .setUserPermissionArray(new TUserPermission[0]);
            }
            ...
            if (permission.getArrayOfGroupPermissions().getGroupPermissionArray() == null) {
                permission.getArrayOfGroupPermissions()
                      .setGroupPermissionArray(new TGroupPermission[0]);
            }
        }
    ...
    private TMetaDataPathDetail list(URI surl, boolean verbose, int offset,
        ...
        SrmLsResponse response = srm.srmLs(
              new SrmLsRequest(null, new ArrayOfAnyURI(new URI[]{surl}), null, null, verbose, false,
                    1, offset,
                    count));
        while (response.getReturnStatus().getStatusCode() == TStatusCode.SRM_REQUEST_QUEUED ||
            ...
            SrmStatusOfLsRequestResponse status =
                  srm.srmStatusOfLsRequest(
                        new SrmStatusOfLsRequestRequest(null, response.getRequestToken(), offset,
                              count));
        ...
        checkBulkSuccess(response.getReturnStatus(),
              Collections.singletonList(details.getStatus()));
        return details;
    }

    @Nonnull
    @Override
    public TMetaDataPathDetail[] list(URI surl, boolean verbose)
          throws RemoteException, SRMException, InterruptedException {
        try {
    ...
    private int tuneLsSize(URI surl, boolean verbose)
          throws RemoteException, SRMException, InterruptedException {
    ...
    private TMetaDataPathDetail[] doList(URI surl, boolean verbose)
          throws RemoteException, SRMException, InterruptedException {
        int offset = 0;
    ...
    @Nonnull
    @Override
    public TSupportedTransferProtocol[] getTransferProtocols()
          throws SRMException, RemoteException {
    ...
    public void rmdir(URI lookup, boolean recursive) throws RemoteException, SRMException {
        SrmRmdirResponse response = srm.srmRmdir(
              new SrmRmdirRequest(null, lookup, null, recursive));
        checkSuccess(response.getReturnStatus());
    }
    ...
    @Nonnull
    @Override
    public String[] getSpaceTokens(String userSpaceTokenDescription)
          throws RemoteException, SRMException {
    ...
    @Nonnull
    @Override
    public TMetaDataSpace reserveSpace(long size, @Nullable String description,
          @Nullable TAccessLatency al,
        ...
        SrmReserveSpaceResponse response = srm.srmReserveSpace(
              new SrmReserveSpaceRequest(null, description, new TRetentionPolicyInfo(rp, al),
        ...
        while (response.getReturnStatus().getStatusCode() == TStatusCode.SRM_REQUEST_QUEUED ||
            ...
            SrmStatusOfReserveSpaceRequestResponse status =
                  srm.srmStatusOfReserveSpaceRequest(
                        new SrmStatusOfReserveSpaceRequestRequest(null,
    ...
    @Nonnull
    @Override
    public TMetaDataSpace[] getSpaceMetaData(String... spaceTokens)
          throws RemoteException, SRMException {
