...
public class ChimeraVfs implements VirtualFileSystem, AclCheckable, QuotaVfs {
    ...
    /**
     * Block size used to report and set quotas.
     */
    private final static int QUOTA_BLOCK_SIZE = ByteUnit.GiB.toBytes(1);
    ...
    public rquota getQuota(int id, int type) {
        ...
        Quota qq = switch (type) {
            case 0 -> // user
                  _fs.getQuota().getUserQuotas().get(id);
            case 1 -> // group
                  _fs.getQuota().getGroupQuotas().get(id);
        ...
        Long diskQuota = qq.getReplicaSpaceLimit();
        if (diskQuota == null) {
            quota.rq_active = false;
            return quota;
        }

        int q = (int) (qq.getReplicaSpaceLimit() / QUOTA_BLOCK_SIZE);

        quota.rq_bhardlimit = q;
        quota.rq_bsoftlimit = q;

        quota.rq_bsize = QUOTA_BLOCK_SIZE;
        quota.rq_curblocks = (int)(qq.getUsedReplicaSpace() / QUOTA_BLOCK_SIZE);
        quota.rq_fhardlimit = Integer.MAX_VALUE;
        quota.rq_fsoftlimit = Integer.MAX_VALUE;
        quota.rq_curfiles = Integer.MAX_VALUE;
        quota.rq_btimeleft = (int)_fs.getQuota().nextQuotaRefresh().toSeconds();
        quota.rq_ftimeleft = (int)_fs.getQuota().nextQuotaRefresh().toSeconds();

        return quota;
    }

    @Override
    public rquota setQuota(int id, int type, sq_dqblk sq) {
        ...
        qq.setReplicaSpaceLimit(((long)sq.rq_bhardlimit) * QUOTA_BLOCK_SIZE);
        if (type == 0) {
            _fs.getQuota().setUserQuota(qq);
        } else {
            _fs.getQuota().setGroupQuota(qq);
        }

        quota.rq_bhardlimit = sq.rq_bhardlimit;
        quota.rq_bsoftlimit = sq.rq_bhardlimit;
