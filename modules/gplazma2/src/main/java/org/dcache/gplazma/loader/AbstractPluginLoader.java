...
/**
 * This abstract class contains the common code for all PluginLoader classes. It supports creating
 * fresh plugins (objects of a class that implements GPlazmaPlugin) from some corresponding {@link
 * PluginMetadata}.  The PluginMetadata for a plugin is obtains from a {@link PluginRepository}
 * which, in turn, comes from an abstract {@link PluginRepositoryFactory}.
 * <p>
 * Classes that extend this class to provide a concrete PluginLoader must supply the
 * PluginRepositoryFactory object that will provide the PluginRepository.
 */
