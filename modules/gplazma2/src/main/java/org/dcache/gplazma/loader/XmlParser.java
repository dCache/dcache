...
/**
 * Class that parses the XML configuration file and returns useful information. Plugin XML metadata
 * has a root <tt>plugins</tt> element and zero or more <tt>plugin</tt> child elements. Each
 * <tt>plugin</tt> element has metadata for that plugin as child elements of that <tt>plugin</tt>
 * element.
 * <p>
 * The following is a list of possible child elements (their local-name values) along with the
 * cardinality and a brief description.
 * <table>
 * <tr>
 * <th>local-name</th>
 * <th>cardinality</th>
...
public class XmlParser {
    ...
    /**
     * If the same class is used by multiple plugin descriptions then we cannot know which plugin
     * description is correct. Because of this, we remove all plugin descriptions that use this
     * class.
     */
    ...
    private NodeList buildPluginNodeList() {
        ...
        try {
            ...
            if (genericCause instanceof SAXParseException) {
                ...
                LOGGER.error("Unable to parse plugin metadata: [{},{}] {}", cause.getLineNumber(),
