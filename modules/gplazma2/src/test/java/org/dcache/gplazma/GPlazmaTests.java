...
public class GPlazmaTests {
    ...
    public void setUp() {
        ...
        ACCOUNT_CONFIG_ITEM = new ConfigurationItem(ACCOUNT, REQUISITE, ACCOUNT_NAME,
              accountProperties);
        ...
        SESSION_CONFIG_ITEM = new ConfigurationItem(SESSION, REQUIRED, SESSION_NAME,
              sessionProperties);
    ...
    /**
     * The authenticatie strategy will throw an exception as there are no principals identified
     *
    ...
    /**
     * Test of login method, of class GPlazma. This test should succeed as we configure the four
     * plugins in configuration that will fill in minimally required plincipals and attributes
     * <br>
     * First plugin is authentication type FindPrincipalsInCredentialsPlugin, which should check
     * both public and private credentials and find all objects of type Principal.
     * <br>
     * Second plugin is mapping type IdentityMappingPlugin (self explanatory)
     * <br>
     * Third plugin is account type  which will check that specified uid Principal is present,
     * otherwise it fails It also records found uids in the static set, which we can check to verify
     * that is was called
     * <br>
     * And the last plugin to be called is AddHomeRootSessionPlugin that adds home and root
     * attributes only if certain user principal is present
     */
    @Test
    ...
    /**
     * Configuration is same as above, but we add optional plugins of each type that fail
     *
     * @throws AuthenticationException
     */
    @Test
    ...
    public void testLoginWithRequiredAuthenticationFailed() throws AuthenticationException {
        ...
        // do the work here
        LoginReply result = new GPlazma(newLoadStrategy(config), EMPTY_PROPERTIES).login(
              _inputSubject);
    ...
    /**
     * Configuration is same as in testLogin, but without session should fail in the check of
     * presence of the home and root attributes
     *
     * @throws AuthenticationException
     */
    @Test(expected = AuthenticationException.class)
    ...
    /**
     * Configuration is same as in testLogin, but without authentication should fail in the check of
     * presence identified principlas
     *
     * @throws AuthenticationException
     */
    @Test(expected = AuthenticationException.class)
