...
public class CanlSslSocketCreator extends SocketFactory {
    ...
    private static final Set<String> bannedProtocols = ImmutableSet.of("SSL", "SSLv2", "SSLv2Hello",
          "SSLv3");
    ...
    public Socket createSocket(String host, int port, InetAddress localAddr, int localPort)
          throws IOException {
        SSLSocket socket = (SSLSocket) getSocketFactory().createSocket(host, port, localAddr,
              localPort);
        setCipherSuiteAndProtocol(socket);
        return socket;
    }
    ...
    public Socket createSocket(InetAddress address, int port, InetAddress localAddr, int localPort)
          throws IOException {
        SSLSocket socket = (SSLSocket) getSocketFactory().createSocket(address, port, localAddr,
              localPort);
        setCipherSuiteAndProtocol(socket);
        return socket;
    }
    ...
    public SSLSocketFactory getSocketFactory() throws IOException {
        try {
            ...
            Throwables.propagateIfPossible(e, IOException.class);
            throw new IOException("Failed to create SSL Server Socket Factory: " + e.getMessage(),
                  e);
        }
    }
}
