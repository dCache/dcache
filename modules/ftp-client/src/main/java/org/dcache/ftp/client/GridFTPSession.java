...
public class GridFTPSession extends Session {
    ...
    /**
     * Indicates Extended Block Mode for data transfer. Used with {@link GridFTPClient#setMode(int)
    ...
    /**
     * Indicates that the data channel will carry the raw data of the file transfer, with no
     * security applied. Used with {@link GridFTPClient#setDataChannelProtection(int)
     * setDataChannelProtection()}.
     */
    public static final int PROTECTION_CLEAR = 1;

    /**
     * Indicates that the data will be integrity protected. Used with {@link
     * GridFTPClient#setDataChannelProtection(int) setDataChannelProtection()}.
     */
    public static final int PROTECTION_SAFE = 2;

    /**
     * Indicates that the data will be confidentiality protected (Currently, not support by GridFTP
     * servers). Used with {@link GridFTPClient#setDataChannelProtection(int)
     * setDataChannelProtection()}.
     */
    public static final int PROTECTION_CONFIDENTIAL = 3;

    /**
     * Indicates that the data will be integrity and confidentiality protected. Used with {@link
     * GridFTPClient#setDataChannelProtection(int) setDataChannelProtection()}.
     */
    ...
    /**
     * This concerns local server. if in SERVER_EPAS mode, the server listener socket list is stored
     * here. If in SERVER_PASV mode, the server listener sockets is stored in serverAddress
     * variable.
     */
    ...
    /**
     * In addition to the inherited functionality, this method also (1) checks if extended active /
     * passive server modes are set correctly, (2) checks if Mode E is needed, and if so, checks
     * whether it has been set. If not, ClientException is thrown.
     */
    @Override
    ...
    /**
     * @return true if this session requires GridFTP extensions; false if it only requires vanilla
     * FTP.
     */
