...
/**
 * This is the main user interface for FTP operations. Use this class for client - server or third
 * party transfers that do not require GridFTP extensions. Consult the manual for general usage.
 * <br><b>Note:</b> If using with GridFTP servers operations like
 * {@link #setMode(int) setMode()}, {@link #setType(int) setType()} that affect data channel
 * settings <b>must</b> be called before passive or active data channel mode is set.
 **/
public class FTPClient {
    ...
    /**
     * List of the checksum algorithms supported by the server as described in {@link
     * http://www.ogf.org/documents/GFD.47.pdf [GridFTP v2 Protocol Description]}
     */
    ...
    /**
     * Returns the last reply received from the server. This could be used immediately after the
     * call to the constructor to get the initial server reply
     */
     ...
     * @throws ServerException if the file does not exist or an error occured.
     */
     ...
     * <p>
     * <br><b>Note</b>:<i>
     * This function can only parse Unix ls -d like output. Please note that the LIST output is
     * unspecified in the FTP standard and each server might return slightly different output
     * causing the parsing to fail. Also, if the ftp server does not accept -d option or support
     * wildcards, this method might fail. For example, this command will fail on GridFTP server
     * distributed with GT 4.0.0. It is strongly recommended to use {@link #mlsd() mlsd()} function
     * instead.</i>
     *
     * @return Vector list of {@link FileInfo FileInfo} objects, representing remote files
     * @see #mlsd()
     */
    ...
    /**
     * Performs remote directory listing with the specified filter. Sends 'LIST -d &lt;filter&gt;'
     * command.
     * <p>
     * <br><b>Note</b>: <i>
     * This function can only parse Unix ls -d like output. Please note that the LIST output is
     ...
     * causing the parsing to fail. Also, if the ftp server does not accept -d option or support
     * wildcards, this method might fail. For example, this command will fail on GridFTP server
     * distributed with GT 4.0.0. It is strongly recommended to use {@link #mlsd(String) mlsd()}
     * function instead. </i>
     *
    ...
    /**
     * Performs remote directory listing with the specified filter and modifier. Sends 'LIST
     * &lt;modifier&gt; &lt;filter&gt;' command.
     * <p>
     * <br><b>Note</b>: <i>
     * This function can only parse Unix ls -d like output. Please note that the LIST output is
     * unspecified in the FTP standard and each server might return slightly different output
     * causing the parsing to fail. Also, please keep in mind that the ftp server might not
     * recognize or support all the different modifiers or filters. In fact, some servers such as
     * GridFTP server distributed with GT 4.0.0 does not support any modifiers or filters (strict
     * RFC 959 compliance). It is strongly recommended to use {@link #mlsd(String) mlsd()} function
     * instead.</i>
     *
     * @param filter   "*" for example, can be null.
    ...
    public Vector list(String filter, String modifier)
        ...
        BufferedReader reader =
              new BufferedReader(
                    new StringReader(new String(received.toByteArray(), StandardCharsets.UTF_8)));
    ...
    /**
     * Performs directory listing and writes the result to the supplied data sink. This method is
     * allowed in ASCII mode only.
     * <p>
     * <br><b>Note</b>: <i>
     * Please keep in mind that the ftp server might not recognize or support all the different
     * modifiers or filters. In fact, some servers such as GridFTP server distributed with GT 4.0.0
     * does not support any modifiers or filters (strict RFC 959 compliance). It is strongly
     * recommended to use {@link #mlsd(String, DataSink) mlsd()} function instead.</i>
     *
     ...
     *
     * @param path directory to perform listing of. If null, listing of current directory will be
     *             performed.
     * @return Vector list of {@link FileInfo FileInfo} objects, representing remote files
     */
     ...
     *
     * @return Vector list of {@link MlsxEntry MlsxEntry} objects, representing remote files
     */
    public Vector mlsd()
          throws ServerException, ClientException, IOException {
        return mlsd(null);
     ...
     * Sets transfer type.
     *
     * @param type should be {@link Session#TYPE_IMAGE TYPE_IMAGE}, {@link Session#TYPE_ASCII
     *             TYPE_ASCII}, {@link Session#TYPE_LOCAL TYPE_LOCAL}, {@link Session#TYPE_EBCDIC
     *             TYPE_EBCDIC}
     **/
     ...
     * Sets transfer mode.
     *
     * @param mode should be {@link Session#MODE_STREAM MODE_STREAM}, {@link Session#MODE_BLOCK
     *             MODE_BLOCK}
     **/
    ...
    /**
     * Aborts the current transfer. FTPClient is not thread safe so be careful with using this
     * procedure, which will typically happen in multi threaded environment. Especially during
     * client-server two party transfer, calling abort() may result with exceptions being thrown in
     * the thread that currently perform the transfer.
     **/
    ...
    /**
     * Closes connection. Sends QUIT command and closes connection even if the server reply was not
     * positive. Also, closes the local server. This function will block until the server sends a
    ...
    /**
     * Closes connection. Sends QUIT and closes connection even if the server reply was not
     * positive. Also, closes the local server.
     *
     * @param ignoreQuitReply if true the <code>QUIT</code> command will be sent but the client will
     *                        not wait for the server's reply. If false, the client will block for
     *                        the server's reply.
     **/
     ...
     *
     * @return true if the given feature is supported by remote server, false otherwise.
     */
    ...
    /**
     * Sets remote server active, telling it to connect to the client. setLocalPassive() must be
     * called beforehand.
     **/
    ...
    /**
     * Starts local server in passive server mode, with default parameters. In other words, behaves
     * like setLocalPassive(FTPServerFacade.ANY_PORT, FTPServerFacade.DEFAULT_QUEUE)
     **/
    ...
    /**
     * Changes the default client timeout parameters. In the beginning of the transfer, the critical
     * moment is the wait for the initial server reply. If it does not arrive after timeout, client
     * assumes that the transfer could not start for some reason and aborts the operation. Default
     * timeout in miliseconds is Session.DEFAULT_MAX_WAIT. During the waiting period, client polls
     * the control channel once a certain period, which is by default set to
     ...
     * @param append        enables append mode; if true, data will be appened to the remote file,
     *                      otherwise file will be overwritten.
     * @param mListener     marker listener. Can be set to null.
     */
    ...
    /**
     * Actual transfer management. Transfer is controlled by two new threads listening to the two
     * servers.
     **/
     ...
     * Depending on the command executed it might put the server in a different state from the state
     * the client is expecting.</i>
     *
     ...
     *
     * @param passiveMode if true passive connections will be established. If false, they will not.
     */
     ...
     * @param port    If passive is false, this is the port for the "port" parameter
     * @param mode    The value for the "mode" parameter, or 0 if the parameter should not be
     *                specified
     ...
     * <p>
     * Notice that as a side effect this method may change the local server facade passive/active
     * mode setting. The caller should not rely on this setting after call to get2.
     * <p>
     * Even though the active/passive status of the current session is ignored for the actual
     * transfer, it still has to be in a consistent state prior to calling gridftp2Get.
     *
     ...
     * <p>
     * Notice that as a side effect this method may change the local server facade passive/active
     * mode setting. The caller should not rely on this setting after call to gridftp2Get.
     * <p>
     * Even though the active/passive status of the current session is ignored for the actual
     * transfer, it still has to be in a consistent state prior to calling gridftp2Get.
     *
    ...
    /**
     * Performs third-party transfer between two servers. If possibly, GFD.47 (a.k.a GridFTP2) GET
     * and PUT commands are used.
     *
    ...
    /**
     * Controls whether the client attempts to send an ALLO command before a STOR request during the
     * put/asyncPut calls. This is disabled by default in the FTP client and enabled by default in
     * the GridFTP client. This setting will apply to all subsequent transfers.
     *
    ...
    /**
     * Determines whether this client is configured to send an ALLO command before a STOR request in
     * the put/asyncPut methods.
     */
    ...
    /**
     * According to {@link http://www.ogf.org/documents/GFD.47.pdf [GridFTP v2 Protocol
     * Description]} checksum feature has the following syntax:
     * <pre>
     * CKSUM <algorithm>[, …]
     * </pre>
     * getSupportedCksumAlgorithms parses checsum feauture parms and form a list of checksum
     * algorithms supported by the server
     *
     * @return a list of checksum algorithms supported by the server in the order specified by the
     * server
    ...
    /**
     * implement GridFTP v2 CKSM command from {@link http://www.ogf.org/documents/GFD.47.pdf
    ...
    /**
     * implement GridFTP v2 SCKS command as described in {@link http://www.ogf.org/documents/GFD.47.pdf
     * [GridFTP v2 Protocol Description]}
     * <pre>
     * 5.2 SCKS
