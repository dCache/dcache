 ...
 * </b>
 * This class represents the part of the client responsible for data channel management. Especially
 * when the remote server is in the passive mode, it behaves a lot like a local server. Thus its
 * interface looks very much like a server interface.
 * <br>
 * Current implementation is multithreaded. One thread is used for thread management and one for
 * each transfer (this makes sense in GridFTP parallelism).
 * <br>
 * The public methods can generally be divided into setter methods and active methods. Active
 * methods are setActive(), setPassive(), retrieve(), and store(), and setter methods are the
 * remaining. Setter methods do not generally throw exceptions related to ftp. Settings are not
 * checked for correctness until the server is asked to performed some action, which is done by
 * active methods. So you are safe to cal setXX() methods with any argument you like, until you call
 * one of the "active" methods mentioned above.
 * <br>
 * The managing thread is not started until one of the "active" methods is called: setActive(),
 * retrieve(), or store(). These methods are asynchronous (return before completion) and the action
 * is undertaken by the local manager thread. From this point on, all communication back to the
 * caller is done through unidirectional local control channel. Information is communicated back to
 * the user in form of FTP replies (instances of LocalReply). Generally, the sequence of replies
 * should be the same as when communicating with remote server during the transfer (1xx intermediary
 * reply; markers; final 226). Exceptions are serialized into 451 negative reply.
 **/
public class FTPServerFacade {
    ...
    /**
     * Data channels are operated in multithreaded manner and they pass information (including
     * exceptions) to the user using the local control channel. In the unlikely event that it fails,
     * there is no way to communicate the exception to the user. In such circumstances this method
     * should be called to print the exception directly to console.
     **/
    ...
    /**
     * Use this method to get the client end of the local control channel. It is the only way to get
     * the information of the current transfer state.
     **/
    ...
    public HostPort setPassive(PortRange range, int queue)
        ...
        String address = NetworkUtils.getLocalAddress(
              InetAddress.getByName(remoteControlChannel.getHost())).getHostAddress();
    ...
    /**
     * Asynchronous; return before completion. Connect to the remote server. Any exception that
     * would occure will not be thrown but returned through the local control channel.
     **/
    ...
    /**
     * Asynchronous; return before completion. Start the incoming transfer and store the file to the
     * supplied data sink. Any exception that would occure will not be thrown but returned through
    ...
    /**
     * Asynchronous; return before completion. Start the outgoing transfer reading the data from the
     * supplied data source. Any exception that would occure will not be thrown but returned through
     * the local control channel.
     **/
    ...
    /**
     * Use this as an interface to the local manager thread. This submits the task to the thread
     * queue. The thread will perform it when it's ready with other waiting tasks.
     **/
    ...
    /**
     * This inner class represents a local control channel. One process can write replies using
     * BasicServerControlChannel interface, and the other can read replies using
