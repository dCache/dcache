...
/**
 * A SubtreeStateGuide allows a StateVisitor to visit all of a specific subtree. The class allows
 * descent into all StateComponents that are an ancestor of the subtree root (so allowing the visit
 * to reach the subtree root) and all StateComponents that have the subtree root as an ancestor
