...
/**
 * A MessageHandlerChain allows multiple MessageHandler subclass instances to attempt to process an
 * incoming Message.  This allows easy addition of extra monitoring by receiving additional
 * messages.
 * <p>
 * Zero or more MessageHandler subclass instances are registered with the MessageHandlerChain. When
 * passed an incoming Message, the MessageHandlerChain instance will pass the Message to each
 * MessageHandler subclass instance in turn until one succeeds in processing the Message.
 *
...
public class MessageHandlerChain implements MessageMetadataRepository<UOID>,
    ...
    /**
     * Common method to send a CellMessage and register a handler for the return message. This is
     * deprecated against using Vehicles and registering MessageHandlers.
     *
    ...
    /**
     * For each message we send, a small amount of metadata is recorded (when it was sent and a
     * long). The long is so, when the return message is received, we can pass this parameter on to
     * the message processing plug-in.  The time is so we can (every so often) delete stale entries
     * due to message-loss.
     */
    ...
    /**
     * Scan through our recorded Metadata and remove very old entries. This is only done "every so
     * often" and adds some safety against lost packets resulting in accumulated memory usage.
     */
