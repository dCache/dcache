...
/**
 * Interface for an algorithm that wishes to iterate over the current state. Any algorithm that
 * wishes to operate over the dCache state must implement this interface.
 * <p>
 * Subclasses of StateValues will call the corresponding method.
 * <p>
 * StateComposite objects call visitCompositePreDescend() before visiting their children. Once all
 * children have been visited the StateComposite will call visitCompositePostDescend().
 * <p>
 * For both StateComposite methods, the metadata parameter is either null or references a
 * keyword-value repository of metadata. This metadata is specific for the particular
 * StateComposite. The metadata is persistent and independent of the actual contents of the State
 * tree; it is linked to a nodes path within the state.
 * <p>
 *
