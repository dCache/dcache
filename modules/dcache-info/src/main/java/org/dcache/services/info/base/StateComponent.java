...
/**
 * All nodes within the State composition must implement this interface.  Nodes are either
 * StateComposite class (i.e., branches) or sub-classes of the abstract StateValue (i.e., data).
 * <p>
 * StateComponents may be mortal, ephemeral or immortal.  If mortal, then the
 * <code>getExpiryDate()</code> method specifies when this StateComponent should be removed.
 * Ephemeral
...
interface StateComponent {
    ...
    /**
     * Needed for the Visitor pattern: this method performs actions on the StateVisitor object via
     ...
     *
     * @param path    this parameter informs an StateComponent of its location within the tree so
     *                the visit methods can include this information.
     ...
     */
    void buildTransition(StatePath ourPath, StatePath childPath, StateComponent newChild,
          StateTransition transition) throws MetricStatePathException;
     ...
     * @param transition the StateTransition object within which we should register children to be
     *                   deleted.
     * @param forced     whether we should simply remove our children, or test whether they are to
     *                   be deleted
     */
     ...
     * StateComponent will be removed.  This is equivalent to buildRemovalTransition(...,
     * transition, true) for the top-most element in the named subtree.
     */
    void buildPurgeTransition(StateTransition transition, StatePath ourPath,
