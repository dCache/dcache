...
/**
 * A simple skeleton class that provides a StateVisitor that iterates over items in a list. The
 * constructor should be called with the StatePath of the parent StateComposite.  Each child of this
 * StateComposite is considered a item within the list and newListItem() will be called for each
 * such list.  Subclasses may overload that method.
 * <p>
...
public class SkeletonListVisitor implements StateVisitor {
    ...
    /**
     * Check whether the visitor is within (underneath) the list item.  If so, then getKey() will
