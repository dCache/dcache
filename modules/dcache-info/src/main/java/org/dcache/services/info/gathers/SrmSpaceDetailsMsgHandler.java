...
public class SrmSpaceDetailsMsgHandler implements MessageHandler {
    ...
    public boolean handleMessage(Message messagePayload, long metricLifetime) {
        ...
        for (Space space : spaces) {
            ...
            update.appendUpdate(thisSpacePath.newChild("access-latency"),
                  new StringStateValue(space.getAccessLatency().toString(), metricLifetime));
            update.appendUpdate(thisSpacePath.newChild("retention-policy"),
                  new StringStateValue(space.getRetentionPolicy().toString(), metricLifetime));

            StatePath spacePath = thisSpacePath.newChild("space");
            update.appendUpdate(spacePath.newChild("free"),
                  new IntegerStateValue(space.getAvailableSpaceInBytes(), metricLifetime));
            update.appendUpdate(spacePath.newChild("used"),
                  new IntegerStateValue(space.getUsedSizeInBytes(), metricLifetime));
            update.appendUpdate(spacePath.newChild("allocated"),
                  new IntegerStateValue(space.getAllocatedSpaceInBytes(), metricLifetime));
            update.appendUpdate(spacePath.newChild("total"),
                  new IntegerStateValue(space.getSizeInBytes(), metricLifetime));

            Date creationDate = new Date(space.getCreationTime());
            CellMessageHandlerSkel.addTimeMetrics(update, thisSpacePath.newChild("created"),
                  creationDate, metricLifetime);

            update.appendUpdate(thisSpacePath.newChild("id"),
                  new StringStateValue(String.valueOf(space.getId()), metricLifetime));
            update.appendUpdate(thisSpacePath.newChild("state"),
                  new StringStateValue(space.getState().toString(), metricLifetime));

            Long expirationTime = space.getExpirationTime();
            if (expirationTime != null) {
                update.appendUpdate(thisSpacePath
                            .newChild("lifetime"),
                      new IntegerStateValue(expirationTime - space.getCreationTime(),
                            metricLifetime));
            }

            addLinkgroup(update, thisSpacePath, String.valueOf(space.getLinkGroupId()),
                  String.valueOf(space.getId()), metricLifetime);

            addVoInfo(update, thisSpacePath.newChild("authorisation"), space.getVoGroup(),
                  space.getVoRole(), metricLifetime);
    ...
    private void addVoInfo(StateUpdate update, StatePath parentPath, String group,
        ...
        if (group != null) {
            update.appendUpdate(parentPath.newChild("group"),
                  new StringStateValue(group, metricLifetime));
            ...
            update.appendUpdate(parentPath.newChild("FQAN"),
    ...
    private void addLinkgroup(StateUpdate update, StatePath parentPath, String lgid,
        ...
        update.appendUpdate(parentPath.newChild("linkgroupref"),
              new StringStateValue(lgid, metricLifetime));

        // Add the reference to this space reservation within the corresponding linkgroup
        update.appendUpdate(LINKGROUPS.newChild(lgid).newChild("reservations").newChild(spaceId),
