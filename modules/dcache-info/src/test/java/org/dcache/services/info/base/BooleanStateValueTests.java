...
public class BooleanStateValueTests extends InfoBaseTestHelper {
    ...
    public void testBooleanStateValueBooleanLong() {
        ...
        for (long duration = -1; duration < 3; duration++) {
            assertIsMortal("boolean: false + " + Long.toString(duration),
                  new BooleanStateValue(false, duration), duration < 0 ? 0 : duration);
            assertIsMortal("boolean: true + " + Long.toString(duration),
