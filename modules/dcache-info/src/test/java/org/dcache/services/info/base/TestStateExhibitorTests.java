...
/**
 * This class provides a set of tests to check the behaviour of {@link TestStateExhibitor}. Some of
 * the tests involve the {@link MalleableStateTransition} which, in turn, is tested using a
...
public class TestStateExhibitorTests {
    ...
    /**
     * Add a single metric to the {@link TestStateExhibitor} and configure the {@link
     * VerifyingVisitor} so it expects this metric at the same location.
     */
    ...
    /**
     * Add a single metric to an empty {@link TestStateExhibitor}. An empty {@link VerifyingVisitor}
     * is created and told to expect a metric at the same location. The state is visited using
     * {@link TestStateExhibitor#visitState(StateVisitor)} and the VerifyingVisitor is checked to
     * see whether it is satisfied with the result.
     *
     * @param metricValue the metric value to check.
     * @param useClone    if true then visitClonedState() is used instead of visitState()
     */
    ...
    /**
     * Visitor that visits all the state, collects metadata and allows list-assertions to be made
