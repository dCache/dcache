...
public class XmlSerialiserTests {
    ...
    public void setUp() {
        ...
        StatePath domainPath = StatePath.parsePath(
              "domains.\"bothersome\"&annoying<Cells>'sDomain");
        _exhibitor.addListItem(domainPath, "domain", "name");
        StatePath cellsPath = domainPath.newChild("cells");
        _exhibitor.addListItem(cellsPath.newChild("\"Foo & Bar\" <foobar@example.org>'s cell"),
              "cell", "id");
    }
    ...
    private static class AssertingErrorHandler implements ErrorHandler {
        ...
        private String buildMessage(SAXParseException exception) {
            return "[RELAX-NG] " + exception.getLineNumber() + "," + exception.getColumnNumber()
