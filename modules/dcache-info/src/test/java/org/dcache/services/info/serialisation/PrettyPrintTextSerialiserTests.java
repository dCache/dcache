...
/**
 * This class provides a set of unit-tests for the pretty-print serialiser.  It uses the
 * TestStateExhibitor to provide the StateExhibitor back-end storage.
 * <p>
 * Please note that the tests work by comparing actual output with some "hard-coded" expected
 * output:  should a branch has multiple child elements then the hard-coded expected output imposes
 * a preferred ordering on these elements.
 * <p>
 * The StateExhibitor interface makes no guarantee that sibling StateComponents will be visited in
 * any particular order, or even that the same order will be used between successive visits.
 * <p>
 * It is not intended that the PrettyPrintTextSerialiser presents output with a particular ordering:
 * it may serialise the output in whatever order it visits the dCache state. Therefore, the tests
 * are fragile: the ordering may change and, should this happen, the tests will return incorrectly
 * fail.
 * <p>
 * In practice, both the "real" infrastructure and the test infrastructure (TestStateExhibitor) use
 * a HashMap to store information.  The ordering a StateVistor object will encounter is determined
 * by how the HashMap iterator behaves. This, too, makes no guarantees, but in practice, the small
 * set of distinct strings that are used have (deterministically) the stated ordering.
 */
public class PrettyPrintTextSerialiserTests {
    ...
    private static final StatePath MULTIPLE_ELEMENT_PATH_1_1 = SINGLE_ELEMENT_PATH1.newChild(
          ELEMENT_PATH1_1_NAME);
    private static final StatePath MULTIPLE_ELEMENT_PATH_1_2 = SINGLE_ELEMENT_PATH1.newChild(
    ...
    public void testSingleRootBranchMultipleElementSkip() {
        ...
        assertResult("checking single-branch with multiple element skip",
              MULTIPLE_ELEMENT_PATH_1_1);
    }
    ...
    public void testMultipleRootBranchesSingleElementSkip() {
        ...
        assertResult("checking multiple root branches with single element skip",
              SINGLE_ELEMENT_PATH1);
    }
    ...
    public void testMultipleRootBranchesMultipleElementSkip() {
        ...
        assertResult("checking multiple root branches with multiple element skip",
              MULTIPLE_ELEMENT_PATH_1_1);
    }
    ...
    public void testBranchStringMetricWithRootStringMetricNoSkip() {
        ...
        _expectedResult.println(" |  | ");
        _expectedResult.println(
              " |  +--" + ELEMENT_PATH1_1_NAME + ": \"" + branchMetricValue + "\"  [string]");
        _expectedResult.println(" | ");
        _expectedResult.println(
              " +--" + ELEMENT_PATH2_NAME + ": \"" + rootMetricValue + "\"  [string]");
    ...
    public void testTwoBranchStringMetricsNoSkip() {
        ...
        _expectedResult.println("    | ");
        _expectedResult.println(
              "    +--" + ELEMENT_PATH1_1_NAME + ": \"" + branchMetric1Value + "\"  [string]");
        _expectedResult.println(
              "    +--" + ELEMENT_PATH1_2_NAME + ": \"" + branchMetric2Value + "\"  [string]");
    ...
    public void testRootListItem() {
        ...
        _expectedResult.println(" | ");
        _expectedResult.println(
              " +-[" + className + ", " + idName + "=\"" + ELEMENT_PATH1_NAME + "\"]");
    ...
    public void testRootTwoListItems() {
        ...
        _expectedResult.println(" | ");
        _expectedResult.println(
              " +-[" + className + ", " + idName + "=\"" + ELEMENT_PATH2_NAME + "\"]");
