...
public class StateValueTests extends InfoBaseTestHelper {
    ...
    public void testGetExpiryDate() {
        ...
        for (long expTime : EXPIRY_TIMES) {
            ...
            long expectedTime =
                  System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(expTime < 0 ? 0 : expTime);

            assertTrue("time mismatch with getExpiryDate(): " + expDate + " != " + new Date(
                  expectedTime), Math.abs(expDate.getTime() - expectedTime) < TIME_TOLERANCE);
        }
    }
    ...
    public void testHasExpired() {
        ...
        for (long expTime : EXPIRY_TIMES) {
            ...
            assertTrue("hasExpired() for " + expTime + " returned " + testVal.hasExpired()
                  + ", which is the wrong result", testVal.hasExpired() == expectHasExpired);
        }
    }
    ...
    public void testBuildTransition() {
        ...
        try {
            testVal.buildTransition(new StatePath("path"), new StatePath("childPath"),
                  newEphemeralStateValue(), new StateTransition());
    ...
    /**
     * This test may look a bit odd:  the transition has no effect on the visitor. What happens is
     * that any effect the transition may have had is dealt with by the StateComposite immediately
     * before the StateValue.  If a StateValue is called with a StateTransition it is already known
     * that no changes are to take place to this StateValue from the StateTransition.
     */
    @Test
