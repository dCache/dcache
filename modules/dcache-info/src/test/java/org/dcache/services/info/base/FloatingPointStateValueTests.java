...
public class FloatingPointStateValueTests extends InfoBaseTestHelper {
    ...
    public void testGetTypeName() {
        assertEquals("Unexpected type name", "float",
              new FloatingPointStateValue(TEST_FLOATS[0]).getTypeName());
    }
    ...
    public void testFloatingPointStateValueDoubleBoolean() {
        for (double floatVal : TEST_FLOATS) {
            assertIsEphemeral("creating with " + floatVal,
                  new FloatingPointStateValue(floatVal, false));
        }
        for (double floatVal : TEST_FLOATS) {
            assertIsImmortal("creating with " + floatVal,
                  new FloatingPointStateValue(floatVal, true));
        }
    }
        ...
        for (long duration = -1; duration < 3; duration++) {
            for (double floatVal : TEST_FLOATS) {
                assertIsMortal("float: \"" + floatVal + "\" + " + Long
                            .toString(duration), new FloatingPointStateValue(floatVal, duration),
