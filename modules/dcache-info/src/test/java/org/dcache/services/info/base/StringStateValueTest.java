...
public class StringStateValueTest extends InfoBaseTestHelper {
    ...
    public void testAcceptVisitorStatePathStateVisitor() {
        ...
        for (String testString : TEST_STRINGS) {
            ...
            assertEquals("visitor failed to discover string", expectedToStringValue(testString),
                  testVisitor.foundValue);
    ...
    public void testEqualsObject() {
        ...
        for (String firstString : TEST_STRINGS) {
            ...
            for (String secondString : TEST_STRINGS) {
                ...
                if (shouldBeEqual) {
                    assertEquals("two StringStateValues same creating String, first.equals(second)",
                          firstSsv, secondSsv);
                    assertEquals("two StringStateValues same creating String, second.equals(first)",
                          secondSsv, firstSsv);
                } else {
                    assertFalse(
                          "two StringStateValues different creating String, first.equals(second)",
                          firstSsv.equals(secondSsv));
                    assertFalse(
                          "two StringStateValues different creating String, second.equals(first)",
                          secondSsv.equals(firstSsv));
                }
            }
        }
    }
    ...
    public void testStringStateValueString() {
        for (String stringVal : TEST_STRINGS) {
            assertIsEphemeral("creating with a regular string: " + stringVal,
                  new StringStateValue(stringVal));
        }
    }
    ...
    public void testStringStateValueStringBoolean() {
        for (String stringVal : TEST_STRINGS) {
            assertIsEphemeral("creating with a regular string: " + stringVal,
                  new StringStateValue(stringVal, false));
        }
        for (String stringVal : TEST_STRINGS) {
            assertIsImmortal("creating with a regular string: " + stringVal,
                  new StringStateValue(stringVal, true));
        }
    }
    ...
    public void testStringStateValueStringLong() {
        for (long duration = -1; duration < 3; duration++) {
            assertIsMortal("str: (null) + " + Long.toString(duration),
                  new StringStateValue(null, duration), duration < 0 ? 0 : duration);
            assertIsMortal("str: \"\" + " + Long.toString(duration),
                  new StringStateValue("", duration), duration < 0 ? 0 : duration);

            for (String strVal : TEST_STRINGS) {
                assertIsMortal("str: \"" + strVal + "\" + " + Long
                            .toString(duration), new StringStateValue(strVal, duration),
    ...
    public void testToString() {
        for (String stringVal : TEST_STRINGS) {
            ...
            assertEquals("built from " + stringVal, expectedToStringValue(stringVal),
                  value.toString());
        }
    }
