...
public class IntegerStateValueTest extends InfoBaseTestHelper {
    ...
    public void testAcceptVisitorStatePathStateVisitor() {
        ...
        for (long testInteger : TEST_INTEGERS) {
            ...
            assertTrue("visitor failed to discover integer value",
                  testVisitor.foundValue == testInteger);
    ...
    public void testEqualsObject() {
        ...
        for (long firstInteger : TEST_INTEGERS) {
            ...
            for (long secondInteger : TEST_INTEGERS) {
                ...
                if (shouldBeEqual) {
                    assertEquals("two IntegerStateValues same creating long, first.equals(second)",
                          firstIsv, secondIsv);
                    assertEquals("two IntegerStateValues same creating long, second.equals(first)",
                          secondIsv, firstIsv);
                } else {
                    assertFalse(
                          "two IntegerStateValues different creating long, first.equals(second)",
                          firstIsv.equals(secondIsv));
                    assertFalse(
                          "two IntegerStateValues different creating long, second.equals(first)",
                          secondIsv.equals(firstIsv));
                }
            }
        }
    }
    ...
    public void testIntegerStateValueLongBoolean() {
        for (long intVal : TEST_INTEGERS) {
            assertIsEphemeral("creating with integer: " + intVal,
                  new IntegerStateValue(intVal, false));
        }
        for (long intVal : TEST_INTEGERS) {
            assertIsImmortal("creating with integer: " + intVal,
                  new IntegerStateValue(intVal, true));
        }
    }
    ...
    public void testIntegerStateValueLong() {
        for (long intVal : TEST_INTEGERS) {
            ...
            for (long duration : TEST_DURATIONS) {
                ...
                assertIsMortal("IntegerStateValue \"" + intVal + "\" + " + Long.toString(duration),
                      testVal, duration < 0 ? 0 : duration);
            }
