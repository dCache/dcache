(function(document) {
    ...
    app._initiateDownload = function(file)
    {
            ...
            if(authval == `Basic ${window.btoa('anonymous:nopassword')}`) {
                // Treat anonymous sessions as no auth
                authval = null;
            }
            ...
            if(macaroon !== undefined) {
                ...
                macaroonWorker.addEventListener('error', (e) => {
                    macaroonWorker.terminate();
                     app.dispatchEvent(new CustomEvent('dv-namespace-show-message-toast', {
                         detail: {
                             text: 'Download failed: unable to request authorization for this file.'
                         },
                         bubbles: true,
                         composed: true
                     }));
    ...
    window.addEventListener('dv-namespace-open-file', function (e) {
        if (e.detail.file.fileMetaData !== undefined && e.detail.file.fileMetaData.fileType === "DIR") {
            ...
            Polymer.dom.flush();
        } else if (e.target.singleSelection === true) {
            app._initiateDownload(e.detail.file);
        } else if (e.target.multipleSelection === true) {
