 ...
 * <p>
 * "This information is encoded in a Fully Qualified Attribute Name (FQAN), in the following
 * format:
 * <p>
 * <group name>/Role=<role name>/Capability=<capability name>
 * <p>
 * This syntax means that the user holds the role <role name> in the group <group name>. If no
 * specific role is held, the <role name> is NULL. The /Capability=<capability name> part is
 * deprecated and will disappear in the future: conforming applications SHOULD be able to handle
 * FQANs where it is absent and SHOULD NOT rely on its presence.
 * <p>
 * Future compatibility issue: It is possible that in the future a /Role=NULL component may be
 * omitted in its entirety. The same goes for a /Capability=NULL part. Conforming applications
...
public class FQANTest {
    ...
    /**
     * The items that may make up a FQAN.  The VO is always present, but GROUP, ROLE and CAPABILITY
     * are optional.
     */
    ...
    public void testRoleNullAndNoCapabilityEqualsWithoutRole() {
        ...
        for (FQAN fqan : fqansWithoutRoleOrCapability) {
            ...
            assertTrue("Testing " + fqan + " equals " + fqanWithNullRole,
                  fqan.equals(fqanWithNullRole));
            assertTrue("Testing " + fqanWithNullRole + " equals " + fqan,
                  fqanWithNullRole.equals(fqan));
        }
    }
    ...
    public void testGetCapabilityForCapabilityNull() {
        ...
        for (FQAN fqan : fqansWithoutCapability) {
            ...
            assertEquals("Testing " + fqanWithNullCapability, "",
                  fqanWithNullCapability.getCapability());
        }
    }
    ...
    public void testToStringForRoleNullAndSomeCapability() {
        ...
        for (FQAN fqan : fqansWithoutRoleOrCapability) {
            ...
            FQAN fqanWithoutNullRole = new FQAN(fqan + capabilitySuffix);
            FQAN fqanWithNullRole = new FQAN(fqan + "/Role=NULL" + capabilitySuffix);
            assertEquals("Testing " + fqanWithoutNullRole, fqanWithoutNullRole.toString(),
                  fqanWithNullRole.toString());
        }
    }
    ...
    /**
     * Build an array of FQANs by considering the supplied VO name, group name, role name and
     * capability name. The resulting FQANs have the form:
     *
     * <pre>
     *   "/" voName ["/" groupName] ["/Role=" roleName] ["/Capability=" capabilityName]
     * </pre>
     * <p>
     * The allWith argument is used to control the list of FQANs returned. Only those FQANs that
     * include some type corresponding to an element in the allWith Set are included in the returned
     * array; for example, specifying allWith with ROLE as its single element, {ROLE}, returns an
     * array with only those FQANs with a "/Role=" value:
     *
     * <pre>
     ...
     * </pre>
     * <p>
     * The allWithout argument further controls the output so that the returned array of FQANs will
     * not include any that have an FqanElement included in the allWithout Set.
     */
