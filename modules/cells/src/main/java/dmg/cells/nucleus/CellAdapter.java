...
/**
 * The CellAdapter builds the basic implementation of a Cell. The CellAdapter implements all
 * required methods of the Cell interface and performs some basic actions as long as those methods
 * are not overloaded by a subclass of CellAdapter. CellAdapter introduces a new set of methods
 * which provide a similiar functionallity as the original Cell callbacks, but are much easier to
 * use. Additionally CellAdapter offers a subset of the CellNucleus methods to be called directly
 * without storing the corresponding handle to the CellNucleus. CellAdapter has a buildin command
 * interpreter
 * <code>see CommandInterpreter</code> which is automatically
 * invokes for each packet containing a plain String object. The result is returned to the sender.
 * CellAdapter forwards and answers ping requests without informing the subclass of CellAdapter as
 * long as this capabitity is not switched of explicitly.
 *
...
public class CellAdapter
    ...
    /**
     * Creates a Cell and the corresponding CellNucleus with the specified name. An extra boolean
     * argument 'startNow' allows to delay the arrival of messages until the CellAdapter.start()
     * method is called.
     *
     * @param cellName is the name of the newly created cell. The name has to be unique within the
     *                 context of this CellDomain.
     * @param args     an arbitrary argument string with can be obtained by getArgs later on.
     * @throws IllegalArgumentException is thrown if the name is not unique within this CellDomain.
     */
    ...
    /**
     * starts the delivery of messages to this cell and executes the auto and defined Setup context.
     * (&lt;cellName&gt;Setup and "!&lt;setupContextName&gt;) Failures to start the cell due to
     * external influences are indicated by a CommandException; all other exceptions are treated as
     * bugs.
     */
     ...
     */
    protected Serializable executeCommand(CommandExecutor command, Args args)
          throws CommandException {
        return command.execute(args);
    }

    /**
     * Executes the ContextVariable : &lt;cellName&gt;Setup and "!&lt;setupContextName&gt;"
     */
    ...
    /**
     * returns an Args object created from the second argument of the constructor : this(String
     * name, String args).
     *
    ...
    /**
     * enables or disables the buildin command interpreter. The default behaviour is to use the
     * interpreter.
     *
     * @param use enables the interpreter if set to 'true' otherwise the interpreter  is disabled.
     */
    ...
    /**
     * enables or disables the ability to answer or to forward a ping request without calling
     * 'messageArrived' or 'messageToForward'. The default behaviour is to answer or to forward a
     * ping.
     *
    ...
    /**
     * returns the CellNucleus assigned to this cell. This handle might be usefull to have access to
     * the full nucleus functionallity.
     *
    ...
    /**
     * Setup the logging context of the calling thread. Threads created from the calling thread
     * automatically inherit this information.
     */
    ...
    /**
     * informs the CellCore to remove this cell. The cell kernel will start the kill sequence as
     * soon as possible.
     */
    protected void kill() {
        _nucleus.kill();
    ...
    /**
     * Returns a Apache Curator instance for this cell. The Curator maintains the CDC of the caller
     * and any callbacks without an explicit executor are delivered on the cell message executor.
     * <p>
     * Note that killing a cell does not remove ephemeral nodes created by the cell. Such nodes are
     * bound to the lifetime of the domain and thus a cell should remove such nodes when it is
     * killed.
     */
    ...
    /**
     * Let's this cell consume messages from a particular named queue. Messages posted to the queue
     * will be delivered to the cell like any other message.
     */
    ...
    /**
     * Subscribes this cell to a specific publish-subscribe topic. Messages posted to the topic will
     * be delivered to the cell like any other message.
     */
    ...
    /**
     * Returns a reader of the specified context Object. The method allows to read throw a 'context
            ...
            switch (flag) {
                case RETRY_ON_NO_ROUTE_TO_CELL:
                    throw new IllegalArgumentException(
                          "RETRY_ON_NO_ROUTE_TO_CELL can only be used with a callback.");
                case PASS_THROUGH:
    ...
    /**
     * should be overwrite to provide a more specific one line information about this cell.
     *
    ...
    /**
     * should be overwrite to provide more specific information about this cell.
     *
     * @param printWriter the printWriter which has to be used to write the information to.
     */
    ...
    public CellInfo getCellInfo() {
        return _nucleus.getCellInfo();
    }

    /**
     * has to be overwritten to receive arriving messages. The LastMessageEvent is filtered out and
     * starts the kill sequence which calls 'cleanUp' at the end of the sequence. If the
     * CommandInterpreter facility is enabled, all string messages are send to the command
     * interpreter and answered without intervention of the callback. If the command could not be
     * found by the CommandInterpreter,
     * <link>dmg.cells.nucleus.CellAdapter#commandArrived(CellMessage)</link>
     * is called if it is overwritten by one of the CellAdapters subclasses. This callback is only
     * used to inform about messages of which the current cell is the final destination. Other
     * messages are delivered throu <code>messageToForward</code>.
     *
    ...
    /**
     * has to be overwritten to receive arriving messages which are not directly addressed to this
     * cell. The default behaviour is to select the next destination and to resend the message.
     *
     * @param msg the reference to message arrived.
     */
    ...
    /**
     * If overwritten this method delivers commands which produced a syntax error which intereted by
     * the CommandInterpreter. The original message string is provides together with a help text
     * offered by the interpreter. If not overwritten this helptext is send back to the caller.
     *
     * @param str is the orginal command string.
     * @param cse is the syntax error exception thrown by the command interpreter. cse.getHelpText
     *            offers the possible help text.
     * @return the object which is send back to the caller. If <code>null</code> nothing is send
     * back.
     */
    ...
    /**
     * Has to be overridden to perform any actions before this cell is started. {@code starting} is
     * called before the first message arrives.
     * <p>
     * If an exception is thrown, the cell will not be started and stopped will be invoked.
     */
    protected void starting() throws Exception {
    }

    /**
     * Has to be overridden to perform any actions after this cell is started. At this point message
     * delivery has begun and the cell can receive requests and replies from other cells.
     */
    protected void started() {
    }

    /**
     * Has to be overridden to perform any action before this cell is destroyed. {@code stopping} is
     * called before the cell is unpublished.
     */
    protected void stopping() {
    }

    /**
     * Has to be overridden to perform any actions before this cell is destroyed. {@code stopped} is
     * called after the last message has arrived.
     */
    ...
    public void postStartup(StartEvent event) {
        for (String queue : Splitter.on(",").omitEmptyStrings()
              .split(_args.getOption("consume", ""))) {
            consume(queue);
        }
        for (String topic : Splitter.on(",").omitEmptyStrings()
              .split(_args.getOption("subscribe", ""))) {
            subscribe(topic);
        }
    ...
    /**
     * belongs to the Cell Interface. Is never called.
     */
    @Override
    ...
    /**
     * belongs to the Cell Interface. If this method is overwritten, the getInfo(PrintWriter pw) is
     * never called.
     */
    @Override
        ...
        if (msg.isFinalDestination()) {
            if (!msg.isReply() && msg.getLocalAge() > msg.getAdjustedTtl()) {
                _log.warn(
                      ...
                      (obj instanceof CharSequence) ? '\'' + Ascii.truncate((CharSequence) obj, 50,
            ...
            if (_useInterpreter && (!msg.isReply()) &&
                ...
                try {
                    ...
                    o = e;
                    _log.error("Command failed due to a bug, please contact support@dcache.org.",
                          e);
    ...
    protected void checkAclPermission(String user, Object command, String acl)
          throws CommandException {
        ...
        try {
            ...
            if (reply == null) {
                throw new CommandException(
                      "Error in acl handling : Acl Request timed out (" + _aclPath + ')');
            }
    ...
    private class RetryingCellMessageAnswerable implements CellMessageAnswerable, Runnable {
        ...
        public RetryingCellMessageAnswerable(CellMessage msg, CellMessageAnswerable callback,
              Executor executor,
