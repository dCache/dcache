...
public final class CellMessage implements Cloneable, Serializable {
    ...
    /**
     * Returns the number of milliseconds since this message was received by the local domain. If
     * the message created in the local domain, then the method returns the number of milliseconds
    ...
    /**
     * Returns the adjusted TTL of a message. The adjusted TTL is the TTL with some time subtracted
     * to allow for cell communication overhead. Returns Long.MAX_VALUE if the TTL is infinite.
     */
