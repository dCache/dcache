...
public class SystemCell
    ...
    private void shutdownCells(List<String> cells, long softTimeout, long hardTimeout) {
        ...
        /* Kill all the cells.
         */
        Map<String, CompletableFuture<?>> futures = cells.stream()
              .collect(toMap(name -> name, _nucleus::kill));

        /* And attach the listener.
         */
        futures.forEach((name, future) -> future.thenRunAsync(listeners.apply(name),
              MoreExecutors.directExecutor()));

        /* Now wait.
         */
        try {
            CompletableFuture<Void>[] futuresAsArray = futures.values()
            ...
            try {
                ...
                CompletableFuture.allOf(futuresAsArray)
                      .get(hardTimeout - softTimeout, TimeUnit.MILLISECONDS);
            }
