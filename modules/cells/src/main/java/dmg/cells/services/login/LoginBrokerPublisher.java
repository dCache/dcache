    ...
    implements CellCommandListener, CellMessageReceiver, CellEventListener, CellLifeCycleAware, CellInfoProvider
{
    ...
    {
        private List<InetAddress> _previous;
        ...
        public List<InetAddress> get()
        {
            ...
            try {
                ...
                Stopwatch stopwatch = _log.isDebugEnabled() ? Stopwatch.createStarted() : null;
                try {
        ...
        {
            boolean hasChanged = _previous != null && !_previous.equals(addresses);

            if (hasChanged) {
                List<InetAddress> removed = new ArrayList<>(_previous);
                removed.removeAll(addresses);
                boolean removing = !removed.isEmpty();

                List<InetAddress> added = new ArrayList<>(addresses);
                added.removeAll(_previous);
            ...
            if (_previous == null || hasChanged) {
                _previous = new ArrayList<>(addresses);
            }
        }

        private static void describeList(StringBuilder sb, List<InetAddress> addresses)
        {
            String list = addresses.stream()
                    .map(AnyAddressSupplier::toString)
        ...
        private static String toString(InetAddress a)
        {
            String name = a.getHostName();
            if (InetAddresses.isInetAddress(name)) {
                return InetAddresses.toUriString(a);
            } else {
