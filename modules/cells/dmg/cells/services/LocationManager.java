   ...
   public class Server implements Runnable {
      private class NodeInfo {
          ...
          public String toString(){
             ...
             if( _listen ){
                 sb.append( "l" );
                 if( _address != null )sb.append(":").append(_address) ;
             }else{
                 sb.append("nl");
             }
      ...
      private HashMap        _nodeDb = new HashMap() ;
      private int            _port   = 0 ;
      private DatagramSocket _socket = null ;
...
//            loadPersistentMap() ;
         ...
         say("Persistent map file set to : "+_permFile);
         return; 
      }
      private synchronized void loadPersistentMap() throws Exception {
         ...
         while( i.hasNext() ){
            ...
            String address = (String)hm.get(node) ;
      ...
      private synchronized void savePersistentMap() throws Exception {
         ...
         ObjectOutputStream out = new ObjectOutputStream( 
                                       new FileOutputStream( _permFile ) ) ;
         try{
            out.writeObject( hm ) ;
         }catch(Exception e){
             esay("Problem writing persistent map "+e.getMessage() );
             _permFile.delete() ;
         }finally{
      ...
      private void prepareSetup( String setupFile , String setupMode ) throws Exception {
         ...
         try{
            ...
            esay("Can't check setup file <"+_setupFileName+"> : "+ee.getMessage() ) ;
            esay(ee);
            throw ee ;
         }
      ...
      public void getInfo( PrintWriter pw ){
         pw.println( "         Version : $Id: LocationManager.java,v 1.10 2005-11-06 07:51:23 patrick Exp $") ;
      ...
      public void run(){
         ...
         while( true ){
            try{
               ...
               esay("Exception in Server receive loop (exiting)");
               esay(ie);
               break ;
            }
            try{
      ...
      public void process( DatagramPacket packet )throws Exception{
          ...
          if( datalen <= 0 ){
             esay( "Empty Packet arrived from "+packet.getAddress() ) ;
             return ;
          }
          String message = new String( data , 0 , datalen ) ;
          say( "server query : ["+address+"] "+"("+message.length()+") "+message) ;
          
          message = (String)command( new Args( message ) ) ;
          
          say( "server reply : "+message ) ;
      ...
      public String hh_listen = "<listenDomainName> [...]" ;
      ...
      public String ac_whatToDo_$_1( Args args ){
         ...
         String serial = ( tmp = args.getOpt("serial") ) != null ?
                         ( "-serial="+tmp ) : "" ;
         return "do "+serial+" "+info.toString() ;
      }
   ...
   public class Client implements Runnable {
      ...
      private Client( InetAddress address , int port , Args args ) throws Exception {
         ...
         _receiver  = _nucleus.newThread(this,"Receiver") ;
         _receiver.start() ;
      ...
      public void getInfo( PrintWriter pw ){
         ...
         pw.println( "RequestsReceived : "+_requestsReceived ) ;
         pw.println( "    RequestsSent : "+_requestsSent ) ;
         pw.println( " RepliesReceived : "+_repliesReceived ) ;
         pw.println( "     RepliesSent : "+_repliesSent) ;
         pw.println( "     Exceptions  : "+_totalExceptions) ;
      }
      public String toString(){
      ...
      public String ac_where_is_$_1( final Args args ){
         final CellMessage msg = getThisMessage() ;
         _requestsReceived++ ;
         _nucleus.newThread( 
             new Runnable(){
                 public void run(){
                   try{
                      msg.setMessageObject(
                          askServer( "whereIs "+args.argv(0) , 4000 )
                                          ) ;
                      msg.revertDirection() ;
                      sendMessage(msg);
                      _repliesSent++;
                   }catch(Exception ee){
                      esay("Problem in 'whereIs' request : "+ee ) ; 
                      _totalExceptions ++ ;
                   }
                 }
             }
      ...
      //
      public String ac_listening_on_$_2( final Args args ){
         final CellMessage msg = getThisMessage() ;
         _requestsReceived++ ;
         _nucleus.newThread( 
             new Runnable(){
                 public void run(){
                   try{
                      _registered = InetAddress.getLocalHost().getHostName()+
                                    ":"+args.argv(1) ;
                                    
                      msg.setMessageObject(
                          askServer( "listeningOn "+
                                     getCellDomainName()+" "+_registered ,
                                     4000 )
                                          ) ;
                      msg.revertDirection() ;
                      _repliesSent++;
                      ...
                      esay("Problem in 'listening on' request : "+ee ) ; 
                      _totalExceptions ++ ;
                   }
                 }
             }
      ...
      private String askServer( String message , long waitTime )
         ...
         long now    = 0 ;
         DatagramPacket packet = null ;
         say( "Sending to "+_address+":"+_port+" : "+new String(data,0,data.length));
         synchronized(  b ){
      ...
      private void startListener() throws Exception {
         ...
         String cellArgs  = "0 "+cellClass+" -prot=raw -lm="+getCellName()+( _useNio ? " -niochannel=true" : "" ) ;
      ...
      private void startConnector( String remoteDomain ) throws Exception {
         String cellName  = "c*" ;
         String cellClass = "dmg.cells.network.LocationMgrTunnel" ;
         String cellArgs  = remoteDomain +" "+ getCellName() +( _useNio ? " -niochannel=true" : "" );
      ...
      public void run(){
         if( Thread.currentThread() == _whatToDo ){
             ...
             while( true ){
                _state ++ ;
                try{
                   String reply = askServer( request , 5000 ) ;
                   say( "whatToDo got : "+reply ) ;
                   ...
                   esay( _toDo = "whatToDo : interrupted" ) ;
                   break ;
                }catch(Exception ee ){
                   esay(_toDo = "whatToDo : excpetion : "+ee ) ;
                }
                try{
                   Thread.sleep(10000) ;
                }catch(InterruptedException iie ){
                   esay(_toDo = "whatToDo : interrupted sleep") ;
                   break ;
                }
             }
             say( "whatToDo finished" ) ;
             
         }else if( Thread.currentThread() == _receiver ){
            DatagramPacket packet = null ;
            while( true ){
               try{
                  packet = new DatagramPacket( new byte[1024] , 1024 ) ;
                  ...
                  if( ( data == null ) || ( packLen == 0 ) )
                    throw new
                    Exception("Zero packet received");
                  Args a = new Args( new String(data,0,packLen) ) ;
                  String tmp = a.getOpt("serial") ;
                  if( tmp == null )return ;
                  Integer s = new Integer( tmp ) ;
                  StringBuffer b = (StringBuffer)_map.get(s) ;
                  if( b == null )
                      throw new
                      Exception( "Not waiting for "+s ) ;

                  b.append( a.toString() ) ;
                  say("Reasonable reply arrived ("+s+") : "+b);
                  synchronized(b){
                     b.notifyAll() ; 
                  }
                  
               }catch(InterruptedException ie ){
                  esay("Receiver Thread interrupted" ) ;
                  break ;
               }catch(InterruptedIOException iioe ){
                  esay("Receiver Thread interrupted (io)" ) ;
                  break ;
               }catch(SocketException se){
                  esay("Receiver Socket problem : "+se ) ;
                  break ;
               }catch(Exception e){
                  esay("Receiver IO problem : "+e ) ;
               }
            }
            say("Receiver thread finished");
         }      
