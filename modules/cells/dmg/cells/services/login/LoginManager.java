// $Id: LoginManager.java,v 1.29.2.8 2007-10-19 09:48:13 radicke Exp $
//
...
public class       LoginManager 
  ...
  private HashMap      _childHash         = new HashMap() ;
  ...
  */
  public LoginManager( String name , String argString ) throws Exception {
      ...
      try{
         ...
         // get the listen port.
         // 
         int listenPort    = new Integer( args.argv(0) ).intValue() ;
         args.shift() ;
         //
         // which cell to start
         //
  ...
  public class LoginBrokerHandler implements Runnable {
     ...
     public Object ac_get_children( Args args ){
        ...
        synchronized( _childHash ){
           Iterator i = _childHash.keySet().iterator() ;
           if( binary ){
              String [] list = new String[_childHash.size()] ;
              list = (String []) _childHash.keySet().toArray(list);
  ...
  private class LoginEventListener implements CellEventListener {
     ...
     public void cellDied( CellEvent ce ) {
        synchronized( _childHash ){
       	 ...
       	 *  if there is no maping for this cell, we create a 'dead' mapping, which will 
       	 *  allow followinf put to identify it as a 'dead' and remove it.
       	 *  
       	 */
  ...
  private class ListenThread implements Runnable {
     ...
     private void openPort() throws Exception {
        ...
        if( ssf == null ){
           ...
           Class     ssfClass = Class.forName((String)list.remove(0));
           Object [] args     = null ;
           ...
           Method meth = ssfClass.getMethod("createServerSocket",methodArgClass) ;
               args[0] = new Integer(_listenPort);
         ...
         if( _isDedicated ) {
             ...
             try {
                 ...
                 while( ifList.hasMoreElements() ) {
                     ...
                     NetworkInterface ne = (NetworkInterface)ifList.nextElement();
                     
                     Enumeration ipList = ne.getInetAddresses();
                     while( ipList.hasMoreElements() ) {
                        InetAddress ia = (InetAddress)ipList.nextElement();
                        // curretntly we do not handle ipv6
                 ...
                 addresses = (InetAddress[])v.toArray( new InetAddress[ v.size() ] );
     ...
     public void run(){
         ...
         while( true ){
            ...
            if( ( _maxLogin > 0 ) && ( currentChildHash > _maxLogin) ){
               ...
               esay( "Connection denied "+currentChildHash+" > "+_maxLogin ) ;
            ...
            _nucleus.newThread( 
                ...
                "protocol"+_connectionRequestCounter     ).start() ;
         }
     ...
     public synchronized void shutdown(){
        ...
        if( _serverSocket.getChannel() == null ){
           ...
           try{ 
              ...
              esay( "ServerSocket faked connect : "+ee  ) ;
           }
        }
  ...
  private class RunEngineThread implements Runnable {
     ...
     public void run(){
       ...
       try{
          ...
          CellAdapter cell = (CellAdapter)_loginConstructor.newInstance( args ) ;
          if( _loginPrintMethod != null ){
             try{
                Object [] a = new Object[1] ;
                a[0] = new Integer( _nucleus.getPrintoutLevel() ) ;
          ...
          say("Exception in secure protocol : " + e);
          esay(e);
          _loginFailures++;
          synchronized( _childHash ){ _childCount -- ; }
       }
