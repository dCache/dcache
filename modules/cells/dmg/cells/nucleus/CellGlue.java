...
class CellGlue {
   ...
   private Hashtable _cellList            = new Hashtable() ;
   private Hashtable _cellEventListener   = new Hashtable() ;
   private Hashtable _killedCellList      = new Hashtable() ;
   private Hashtable _cellContext         = new Hashtable() ;
   ...
   CellGlue( String cellDomainName ){
      ...
      if( ( _cellDomainName == null ) || ( _cellDomainName.equals("") ) )
        _cellDomainName  = "*" ;
      
      if( _cellDomainName.charAt( _cellDomainName.length() - 1 ) == '*' ){
          _cellDomainName = 
             _cellDomainName.substring(0,_cellDomainName.length())+
             new Date().getTime() ;
   ...
   Dictionary       getCellContext(){ return _cellContext ;} 
...
//     info.setCellDomainName( _cellDomainName ) ;
   ...
   synchronized CellTunnelInfo [] getCellTunnelInfos(){
      Enumeration e = _cellList.elements() ;
      Vector v = new Vector() ;
      
      for( ; e.hasMoreElements() ; ){
         Cell c = ((CellNucleus)e.nextElement()).getThisCell() ;
         if( c instanceof CellTunnel ){
            v.addElement( ((CellTunnel)c).getCellTunnelInfo() ) ;
         }
      
      }
      CellTunnelInfo [] info = new CellTunnelInfo[v.size()] ;
      v.copyInto( info ) ;
      return info ;
   ...
   synchronized String [] getCellNames(){
      ...
      int i ;
      e = _cellList.keys() ;
      for( i = 0 ; ( i < size ) && e.hasMoreElements() ; i++ )
         o[i] = (String) e.nextElement() ;
     
      if( i == size )return o ;
      
      e = _killedCellList.keys() ;
      for(  ; ( i < size ) && e.hasMoreElements() ; i++ )
         o[i] = (String) e.nextElement() ;
     
      return o ;
      
   }
   synchronized int getUnique(){ return _uniqueCounter++ ; }
   
   CellInfo getCellInfo( String name ){
      CellNucleus nucleus = (CellNucleus)_cellList.get( name ) ;
      if( nucleus == null ){
         nucleus = (CellNucleus)_killedCellList.get( name ) ;
   ...
   private void sendToAll( CellEvent event ){
      ...
      //
      Enumeration       e , ve ;
      CellEventListener hallo ;
      Vector            v ;
      
      for( e = _cellEventListener.elements() ; 
           e.hasMoreElements() ; ){

         if( ( v = (Vector)e.nextElement() ) == null )continue;
         for( ve = v.elements() ; ve.hasMoreElements() ; ){
            hallo = (CellEventListener)ve.nextElement() ;
            if( hallo == null ){
...
//       sendToAll( new CellEvent( name , CellEvent.CELL_DIED_EVENT ) ) ;
   ...
   private synchronized void _kill( CellNucleus source , 
       ...
       CellNucleus destNucleus = (CellNucleus) _cellList.remove( cellToKill ) ;
   ...
   void   sendMessage( CellNucleus nucleus ,
      ...
      say( "sendMessage : "+msg.getUOID()+" send to "+destination);
      //
      ...
      for( int iter = 0 ; iter < MAX_ROUTE_LEVELS ; iter ++ ){
         ...
         //
         CellNucleus destNucleus = (CellNucleus)_cellList.get( cellName ) ;
...
//                       " forwarded addToEventQueue of "+cellName ) ;
   ...
   void addCellEventListener( CellNucleus nucleus , CellEventListener listener ){
      Vector v = null ;
      if( ( v = (Vector)_cellEventListener.get( nucleus.getCellName() ) ) == null ){
         _cellEventListener.put( nucleus.getCellName() , v = new Vector() ) ;
      }
