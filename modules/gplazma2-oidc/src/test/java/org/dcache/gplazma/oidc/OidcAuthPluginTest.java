...
public class OidcAuthPluginTest {
    ...
    private Set<Principal> whenOidcPluginCalledWith(JsonNode discoveryDoc,
        ...
        Mockito.doReturn(discoveryDoc).when(httpClient).doGet(Mockito.any(URI.class));
        Mockito.doThrow(e).when(httpClient)
              .doGetWithToken(Mockito.anyString(), Mockito.anyString());
    ...
    private Set<Principal> whenPluginCreated(BearerTokenCredential token)
    ...
    public static Matcher<Iterable<? super OidcSubjectPrincipal>> hasSubject(String sub,
          String op) {
