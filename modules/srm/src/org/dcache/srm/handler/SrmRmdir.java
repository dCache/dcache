...
//
// $Id: SrmRmdir.java,v 1.9.2.1 2006-12-21 18:10:16 litvinse Exp $
// $Author: litvinse $
//
...
// 			response.setReturnStatus(returnStatus);
    ...
    private void getDirectoryTree(RequestUser user,
            ...
            Vector tree,
            SrmRmdirRequest request,
            SrmRmdirResponse response) throws SRMException {
        
        if (response.getReturnStatus().getStatusCode() == TStatusCode.SRM_FAILURE) return;
        String[] dirList;
        tree.add(directory);
        try {
            ...
            response.getReturnStatus().setExplanation(directory +" : "+srme.getMessage());
            return;
        } catch (Exception e) {
            esay(e);
            response.getReturnStatus().setStatusCode(TStatusCode.SRM_FAILURE);
            response.getReturnStatus().setExplanation(directory +" : "+e.toString());
            return;
        }
        if ( dirList == null ) {
            return;
        ...
        if ( dirList.length == 0 ) {
            ...
            if ( request.getRecursive() != Boolean.TRUE) {
                response.getReturnStatus().setStatusCode(TStatusCode.SRM_FAILURE);
                response.getReturnStatus().setExplanation("non empty directory, no recursion flag specified ");
                return;
            } else {
                for (int i=0; i<dirList.length; i++) {
                    getDirectoryTree(user,directory+"/"+dirList[i],tree,request,response);
                }
            }
        }
    }
    ...
    public static final  SrmRmdirResponse getFailedResponse(String error,TStatusCode statusCode) {
        if(statusCode == null) {
    ...
    public SrmRmdirResponse srmRmdir() throws SRMException,org.apache.axis.types.URI.MalformedURIException {
        SrmRmdirResponse response  = new SrmRmdirResponse();
        TReturnStatus returnStatus = new TReturnStatus();
        ...
        org.apache.axis.types.URI surl = request.getDirectoryPath();
        say("SURL[0]="+surl);
	...
	//
	getDirectoryTree(user,path,tree,request,response);
	...
	try {
                ...
                esay(srme);
                response.getReturnStatus().setStatusCode(TStatusCode.SRM_FAILURE);
                response.getReturnStatus().setExplanation(path+" "+srme.getMessage());
                return response;
	}
