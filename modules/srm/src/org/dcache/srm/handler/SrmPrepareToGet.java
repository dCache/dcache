    ...
    throws SRMException,org.apache.axis.types.URI.MalformedURIException {
        ...
        for (int i = 0; i < fileRequests.length ; ++i ) {
            ...
            if( request.getDesiredTotalRequestTime() != null ) {
                long nextLifetime = (long)request.getDesiredTotalRequestTime().intValue();
                if( nextLifetime <= 0) {
                    nextLifetime = configuration.getGetLifetime();
                } else if(nextLifetime >0 && lifetimeInSeconds < nextLifetime) {
                    lifetimeInSeconds = nextLifetime;
                }
            }
        }
        long lifetime =
