// $Id: GridftpClient.java,v 1.11 2006-01-26 04:40:59 timur Exp $
...
public class GridftpClient {
    ...
    private void sendAddler32Checksum(long adler32) 
      throws Exception
    {
            String adler32String =long32bitToHexString( adler32);
            
            org.globus.ftp.vanilla.Reply reply =
            client.getControlChannel().execute(
            new org.globus.ftp.vanilla.Command("SITE CHKSUM "+ adler32String));
            if(org.globus.ftp.vanilla.Reply.isPositiveCompletion( reply)) {
            } else {
                esay("WARNING: checksum setting failed");
            }
	...
	boolean passive_server_mode) throws Exception
    {
        ...
        TransferThread getter = new TransferThread(client,sourcepath,sink,emode,true,size);
        getter.start();
    ...
    boolean passive_server_mode) throws Exception {
        ...
        TransferThread putter = new TransferThread(client,destinationpath,source,emode,false,diskFileLength);
        putter.start();
    ...
    private  class TransferThread implements Runnable {
        ...
        public void run() {
            try {
                if(emode) {
                    if(read) {
                        say("gridFtpRead: starting the transfer in emode from "+path);
                        client.extendedGet(path,size,source_sink,null);
                    }
                    else {
                        say("gridFtpWrite: starting the transfer in emode to "+path);
                        client.extendedPut(path,source_sink,null);
                    }
                }
                else {
                    if(read) {
                    say(" starting a transfer in a stream mode from "+path);
                        client.get(path,source_sink,null);
                    }
                    else {
                    say(" starting a transfer in a stream mode to "+path);
