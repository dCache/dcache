#!/bin/sh
#
# $Id: deploy_srmv2.sh,v 1.17.2.9 2006-12-12 00:21:47 timur Exp $
#
...
execute "$cmd"
cmd="tar --directory=${TOMCAT_BASE_PATH} -xzf ${TOMCAT_COMPAT_ARCHIVE}"
execute "$cmd"
...
# take log4j from tomcat
rm ${SRM_WEBAPP_DIR}/WEB-INF/lib/log4j-1.2.8.jar
cp ${TOMCAT_PATH}/common/lib/log4j-1.2.*.jar ${SRM_WEBAPP_DIR}/WEB-INF/lib/
...
printf "modifying default tomcat SHUTDOWN phrase  and permissions of ${TOMCAT_CONFIG} ..."
chmod go-r ${TOMCAT_CONFIG}
cp -p ${TOMCAT_CONFIG} $tmp
sed -e "s/SHUTDOWN/${RANDOM}${RANDOM}${RANDOM}/g" ${TOMCAT_CONFIG} >$tmp
mv $tmp ${TOMCAT_CONFIG}
echo "Done"
...
if [ "$TOMCAT_HTTP_ENABLED" != "true" ] ; then
  printf "commenting out HTTP CoyoteConnector on port ${TOMCAT_PORT} in ${TOMCAT_CONFIG} ..."
text_to_comment0='<Connector className="org.apache.coyote.tomcat4.CoyoteConnector"
...
printf "commenting out AJP CoyoteConnector on port 8009..."
comment_multiline '<Connector className="org.apache.coyote.tomcat4.CoyoteConnector"
               port="8009" minProcessors="5" maxProcessors="75"
               enableLookups="true" redirectPort="8443"
               acceptCount="10" debug="0" connectionTimeout="0"
               useURIValidationHack="false"
               protocolHandlerClassName="org.apache.jk.server.JkCoyoteHandler"/>' ${TOMCAT_CONFIG}
echo "Done"
