...
public class CachedVOMapTest {
    ...
    public void testContainsFullDNFQAN()
          throws IOException {
        Collection<String> mappedNames = loadFixture(
              TEST_FIXTURE_WITHOUT_WILDCARDS).getValuesForPredicatesMatching(
              new NameRolePair(VALID_DN, VALID_FQAN_LONG_ROLE));
    ...
    public void testContainsFullDNFQANWithWC()
          throws IOException {
        Collection<String> mappedNames = loadFixture(
              TEST_FIXTURE_WITH_WILDCARDS).getValuesForPredicatesMatching(
              new NameRolePair(VALID_DN, VALID_FQAN_LONG_ROLE));
        assertTrue(mappedNames.contains(VALID_USERNAME_RESPONSE));
    ...
    public void testContainsFullDNShortFQAN()
        ...
        Collection<String> mappedNames = loadFixture(
              TEST_FIXTURE_WITHOUT_WILDCARDS).getValuesForPredicatesMatching(
              new NameRolePair(VALID_DN, VALID_FQAN_SHORT_ROLE));
        assertTrue(mappedNames.contains(VALID_USERNAME_RESPONSE));
    }
    ...
    public void testDoesContainsWildcardResponseForInvalidUsernameWithWildcards()
        ...
        Collection<String> mappedNames = loadFixture(
              TEST_FIXTURE_WITH_WILDCARDS).getValuesForPredicatesMatching(
              new NameRolePair(INVALID_USERNAME, ""));
        assertTrue(mappedNames.contains(VALID_WC_USERNAME_RESPONSE));
    }
    ...
    public void testUsernameWithDash()
        ...
        Collection<String> mappedNames =
              loadFixture(TEST_FIXTURE_NDGF).getValuesForPredicatesMatching(
                    new NameRolePair(DN_ANDREJ, FQAN_ATLAS_PROD));
