...
/**
 * Responsible for taking an X509Certificate chain from the public credentials and adding a {@link
 * UserNamePrincipal} based on a mapping for the local storage resource returned from a GUMS/XACML
 * service.<br>
 * <br>
 * <p>
 * The authentication method is an alternative to straight VOMS authentication; it requires that the
 * X509 proxy contain the following VOMS extensions: <br>
 * <br>
 *
 * <ul>
 * <li>VO</li>
...
public final class XACMLPlugin implements GPlazmaAuthenticationPlugin {
    ...
    /**
     * Simple struct to hold the extensions extracted from the certificate chain.
     * <p>
     * Attribute formats are described in [1] "An XACML Attribute and Obligation Profile for
     * Authorization Interoperability in Grids". Relevant parts are quoted below.
     *
     * @author arossi
     */
    private static class VomsExtensions {
        ...
        /**
         * From [1]: This attribute holds the Distinguished Name (DN) of the user. This DN is the
         * subject extracted from the user’s certificate. This attribute is implicitly linked in
         * this profile with subject-x509-issuer attribute. The datatype of this attribute is a
         * string, to accommodate the OpenSSL one-line representation of slash-separated Relative
         * Distinguished Names.
         * <p>
         * We acknowledge that the most commonly used representation for this attribute is the X.500
         * datatype; however, we decide not to use it because the slash-separated representation is
         * the defacto standard in our environment. Tools and services are free to support the
         * subject-x509-id as X.500 datatype besides the OpenSSL online representation. In that case
         * the Datatype MUST be set to X509Name.
         */
        private String _x509Subject;

        /**
         * From [1]: This attribute holds the Distinguished Name (DN) of the CA that signed the end
         * entity user certificate. This DN is extracted from the user’s certificate and it is
         * implicitly linked to the subject-x509 attribute-id. The datatype of this attribute is
         * string, for the same reasons argued in the Subject-x509-id attribute.
         */
        private String _x509SubjectIssuer;

        /**
         * From [1]: VOMS maintains the organizational structure of a VO in hierarchical groups.
         * Users can belong to such groups and can have specific roles for each group. In the
         * Attribute Certificate, the membership to a group with a role is encoded as a Fully
         * Qualified Attribute Name (FQAN). This attribute holds one FQAN from the VOMS Attribute
         * Certificate in the user credentials. Because users typically belong to several groups,
         * this attribute can be set many times to encode all FQAN in the AC. For this profile, the
         * order of the FQAN is not relevant, considering that the primary FQAN of the user is
         * conveyed through the attribute VOMS-Primary-FQAN.
         * <p>
         * The PDP SHOULD perform a direct string match of the VOMS FQAN values when it evaluates an
         * authorization request against its policy. VOMS FQANs have an optional suffix, e.g.
         * /Role=NULL. A PDP COULD implement the VOMS matching rules to ignore these type of
         * suffixes.
         */
        ...
        /**
         * From [1]: This attribute holds the name of the first user Virtual Organization (VO) found
         * in the set of attribute certificates. The user is requesting authorization in virtue of
         * her membership to this VO, project or community. There are two methods for extracting the
         * VO name from an Attribute Certificate (AC): (1) from the “VO” attribute of the VOMS AC;
         * (2) from the left-most slash- separated portion of the Fully Qualified Attribute Names
         * (FQAN) [FQAN] attributes. This attribute contains the name extracted from the VO
         * attribute of the AC (method 1).
         * <p>
         * From our experience multiple simultaneous VO usage has not observed the use case. All the
         * VO specific attributes in VOMS (more of these will follow in the document) are describing
         * the top VO which is represented explicitly in the VOMS-PRIMARY-FQAN attribute. The VOMS
         * FQANs from all the potentially conveyed VOs CAN be expressed in the VOMS-FQAN attribute.
         */
        private String _vo;

        /**
         * From [1]: VOMS-signing-subject holds the DN of the VOMS service that signed the first
         * Attribute Certificate in the user credentials. It is extracted from the “issuer”
         * attribute of the VOMS AC and is implicitly linked in this profile to the
         * VOMS-signing-issuer attribute. As evident by its name, this attribute (and the others
         * with similar names in this profile) is designed to convey information about an
         * authoritative membership service implemented via a VOMS service. Other membership service
         * implementations can still use this profile provided that their concepts can be properly
         * described by the semantics of these attributes.
         */
        private String _vomsSigningSubject;

        /**
         * From [1]: Considering that VOMS ACs are signed by a VOMS certificate, VOMS-signing-issuer
         * holds the DN of the CA that signed that VOMS certificate. This attribute does not provide
         * information about the whole trust chain: it provides only the DN of the CA that issued
         * the first VOMS attribute certificate. VOMS-signing-issuer is implicitly linked in this
         * profile to the VOMS-signing-subject attribute. It can be extracted programmatically using
         * the VOMS API and is not displayed in typical command line tools, like voms-proxy-info.
         */
    ...
    /**
     * Does the work of contacting the XACML server to get a mapping not in the cache.
     *
     * @author arossi
     */
    ...
    @Override
    public void start()
          throws ClassNotFoundException, IOException, CertificateException, KeyStoreException {
        ...
        X509CertChainValidatorExt certChainValidator = new CertificateValidatorBuilder().trustAnchorsDir(
              caDir).build();
        ...
        _targetServiceIssuer = convertFromRfc2253(
              credential.getCertificate().getIssuerX500Principal().getName(), true);
        ...
        sender.setSslContextFactory(
              CanlContextFactory.custom().withCertificateAuthorityPath(caDir).build());
        sender.init();
        ...
        axisConfiguration.deployTransport(HttpClientTransport.DEFAULT_TRANSPORT_NAME,
     ...
     *
     * @throws IllegalArgumentException if the CACHE_LIFETIME is set to <0
     */
    ...
    /**
     * Extracts canonical DNS name of storage resource host from network interfaces.
     *
    ...
    private void configureResourceDNSHostName() throws SocketException {
        ...
        try {
            _resourceDNSHostName = Ordering.natural().onResultOf(NetworkUtils.InetAddressScope::of)
                  .max(addressList).getCanonicalHostName();
        } catch (NoSuchElementException ignored) {
        }
    }

    /**
     * Extracts VOMS extensions from the public credentials and adds them to the running list.
     */
    ...
    private void extractExtensionsFromChain(CertPath certPath,
        ...
        if (eec == null) {
            throw new AuthenticationException(
                  "The checked certificate chain contains only proxy certificates.");
        }
        ...
        if (vomsAttributes.isEmpty()) {
            ...
            for (VOMSAttribute vomsAttr : vomsAttributes) {
                ...
                String vomsSigningIssuer =
                      convertFromRfc2253(
                            vomsAttr.getAACertificates()[0].getIssuerX500Principal().getName(),
                            true);
                ...
                for (String fqan : fqans) {
                    ...
                    primary = false;
                    logger.trace(" {} authenticate, adding voms extensions = {}", this,
                          vomsExtensions);
                    extensionsSet.add(vomsExtensions);
                }
            }
    ...
    /**
     * Convenience wrapper; loops through the set of extension groups and calls out to {@link
     * LoadingCache#get(Object)}.
     *
     * @param login        may be <code>null</code>
     * @param extensionSet all groups of extracted VOMS extensions
     ...
     *
     * @param property as defined by <code>gplazma.xacml.client.type</code>; if
