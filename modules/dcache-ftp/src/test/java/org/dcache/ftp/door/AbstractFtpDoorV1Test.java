...
public class AbstractFtpDoorV1Test {
    ...
    public void setUp() throws Exception {
        ...
        doCallRealMethod().when(door)
              .checkFTPCommand(anyBoolean(), anyInt(), anyString(), anyVararg());
    }
            ...
            return new Statement() {
                ...
                public void evaluate() throws Throwable {
                    try {
                        ...
                        if (checkCode) {
                            assertEquals("Unexpected reply '" + commandException.getCode() + " "
                                        + commandException.getMessage() + "'", _code,
                                  commandException.getCode());
                        } else {
                            fail("Caught unexpected exception FTPCommandException '" + _code + "':'"
    ...
    public void whenRnfrIsCalledForNonExistingFilenameReplyFileNotFound550()
        ...
        doCallRealMethod().when(door).ftp_rnfr(anyString());
        when(pnfs.getPnfsIdByPath("/pathRoot/cwd/" + INVALID_FILE)).thenThrow(
              FileNotFoundCacheException.class);
    ...
    public void EPRTshouldReply200ForValidIP4Arg()
        ...
        door._localSocketAddress = door._proxySocketAddress = new InetSocketAddress(
              forString("::1"), 21);
    ...
    public void whenMkdPermissionDeniedReply550() throws Exception {
        doCallRealMethod().when(door).ftp_mkd(anyString());
        when(pnfs.createPnfsDirectory("/pathRoot/cwd/" + NEW_DIR)).thenThrow(
              PermissionDeniedCacheException.class);
        thrown.expectCode(550);
        door.ftp_mkd(NEW_DIR);
    }
    ...
    public void whenMkdFileExistReply550() throws Exception {
        doCallRealMethod().when(door).ftp_mkd(anyString());
        when(pnfs.createPnfsDirectory("/pathRoot/cwd/" + NEW_DIR)).thenThrow(
              FileExistsCacheException.class);
        thrown.expectCode(550);
        door.ftp_mkd(NEW_DIR);
    }
    ...
    public void whenSiteWhoamiShowUsername() throws Exception {
        subject.getPrincipals()
              .addAll(aSetOfPrincipals().withUid(1234).withUsername("test-user").build());
