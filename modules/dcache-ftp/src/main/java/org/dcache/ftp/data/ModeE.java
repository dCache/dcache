 ...
 * <p>
 * Be aware that it is quite easy to introduce race conditions, so please keep this in mind when
 * making changes. In particular the EOD and EOD count handling is a little tricky.
 */
public class ModeE extends Mode {
     ...
     * <p>
     * Large blocks will reduce the overhead of sending. However, it case of multible concurrent
     * streams, large blocks will make disk access less sequential on both the sending and receiving
     * side.
     */
    ...
    /**
     * Implementation of send in mode E. There will be an instance per data channel. The sender
     * repeatedly bites _blockSize bytes of the file and transfers it as a single block. I.e.
            ...
            switch (_state) {
                case PREPARE_BLOCK:
                    ...
                    if (_count > 0) {
                        ...
                        _header.put((byte) (EOF_DESCRIPTOR | EOD_DESCRIPTOR
    ...
    /**
     * Implementation of receive in mode E. There will be an instance per data channel.
     */
            ...
            if (_count == 0) {
                ...
                if ((_flags & ~KNOWN_DESCRIPTORS) != 0) {
                    throw new FTPException(
                          "Received block with unknown descriptor (" + _flags + ")");
                }
        ...
        switch (_direction) {
            ...
            case Outgoing:
                String percent = getSize() > 0
                      ? (" (" + toThreeSigFig(100 * _currentCount / (double) getSize(), 1000)
                      + "% desired transfer)")
                      : "";
    ...
    public boolean hasCompletedSuccessfully() {
        return _transferStarted && _activeDataChannels.get() == 0
              && _errorDataChannels.longValue() == 0;
    }
}
