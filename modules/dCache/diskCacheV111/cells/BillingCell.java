// $Id: BillingCell.java,v 1.20 2006-02-13 21:21:17 podstvkv Exp $
...
public class BillingCell extends CellAdapter {
    ...
    private HashMap     _poolStatistics = new HashMap() ;
    private HashMap     _poolStorageMap = new HashMap() ;
    ...
    private SimpleDateFormat directoryNameFormat
    ...
    private void dumpPoolStatistics( String name ) throws Exception {
       ...
       try{
         Iterator     pools = _poolStorageMap.keySet().iterator() ;
         
         while( pools.hasNext() ){
         
           String   poolName = pools.next().toString() ;
           HashMap  map      = (HashMap)_poolStorageMap.get(poolName) ;
           Iterator classes  = map.keySet().iterator() ;
           while( classes.hasNext() ){
              String className = classes.next().toString() ;
              long []  counter = (long [])map.get(className) ;
              pw.print(poolName) ;
              pw.print("  ");
              pw.print(className) ;
       ...
       synchronized( _poolStatistics ){
	     ...
	     _poolStatistics.put( cellName , counters = new long[4] ) ;
          ...
          if( ( info instanceof PnfsFileInfoMessage )  ){
             ...
             if( sinfo != null ){
                HashMap map = (HashMap)_poolStorageMap.get(cellName) ;
                if( map == null )
                   _poolStorageMap.put( cellName , map = new HashMap() ) ;
