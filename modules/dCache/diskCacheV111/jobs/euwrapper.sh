...
#	
#
#   wrapper script to wrap eucp using the standard dCache HSM shellscript interface
#   
#
#    dCache configuration
#
#   hsm set osm -command=<fullPathToThisScript>	(e.g. -command=/opt/d-cache/jobs/euwrapper.sh )
#   hsm set osm -eudir=<fullPathToEurogateCapiDirectory>     (e.g -eudir=/opt/d-cache/eurogate/capi )
#   hsm set osm -eudoor=<EurogateMachineName:Port>	(e.g. -eudoor=reagan.desy.de:28000 )
...
yfilename=${2}
    ...
    echo "execute put ${euscript} ${yfilename} ${ygroup}" 1>&2
    ybfid=`${euscript} ${yfilename} ${ygroup}`

#   ybfid=111111111111:22222222222222:aaaaaa
#   echo "fake ${euscript} ${yfilename} ${ygroup}" 1>&2
#
#
#
#
...
euscript=$eudir/eucp 
[ ! -f "${euscript}" ] && problem 4 "${euscript} not found" 

euapp=$eudir/test
[ ! -f "${euapp}" ] && problem 5 "${euapp} not found" 
#
#
# test for optional key '-eudoor'
[ -z "${eudoor}" ] && problem 6 "key 'eudoor' not set"

# this variable must be avaiable to eucp-process
...
#  
   ...
   echo "execute get ${euscript} ${bfid} ${filename}" 1>&2
   ${euscript} ${bfid} ${filename} 1>&2
   if [ $? -ne 0 ] ; then
      errorReport "Copy back failed : ${euscript} ${bfid} ${filename}"
...
#
   ...
   if [ $errno -ne 0 ] ; then
      ...
      eurmscript=$eudir/eurm 
      [ ! -f "${eurmscript}" ] && ln -s ${euscript} ${eurmscript} 1>&2
