// $Id: DCapDoorInterpreterV3.java,v 1.75.2.4 2007-04-02 10:19:39 tigran Exp $
//
...
public class DCapDoorInterpreterV3 implements KeepAliveListener {
    ...
    private PermissionHandler _permissionHandler = null;
    ...
    private boolean getUserMetadata( String userName , String userRole , String provider ){
        ...
        try{
            ...
            try{ _userUid = Integer.parseInt(tmp) ; }catch(Exception e){} ;
            tmp = (String)r[6] ;
            try{ _userGid = Integer.parseInt(tmp) ; }catch(Exception e){} ;
    ...
    public void keepAlive(){
        ArrayList list = null ;
        synchronized( this ){
            list = new ArrayList(_sessions.values()) ;
        }
        Iterator i = list.iterator() ;
        while( i.hasNext() ){
            SessionHandler sh = (SessionHandler)i.next() ;
            try{
                sh.keepAlive() ;
            }catch(Throwable t ){
    ...
    throws Exception {
        ...
        try{
            ...
            synchronized( _messageLock ){
               ...
               _sessions.put( new Integer(sessionId) , se ) ;
            }
    ...
    throws Exception {
        ...
        try{
            ...
            //  (==synchronized)
            synchronized( _messageLock ){
                _sessions.put( new Integer(sessionId) ,
    ...
    throws Exception {
        ...
        _lastCommandTS = new Date() ;
        IoHandler io = (IoHandler)_sessions.get( new Integer(sessionId) ) ;
        if( io == null )
            throw new
    ...
    public Object ac_get_door_info( Args args ){
        ...
        ArrayList list = new ArrayList(_sessions.size()) ;
        synchronized( this ){
            Iterator i = _sessions.values().iterator() ;
            while( i.hasNext() ){
                Object o = i.next() ;
                if( ! ( o instanceof IoHandler ) )continue ;
                list.add( ((IoHandler)o).getIoDoorEntry() ) ;
            }
        }
        info.setIoDoorEntries( (IoDoorEntry[])list.toArray(new IoDoorEntry[0]) );
    ...
    public String ac_retry_$_1( Args args ) throws Exception {
        ...
        if( ( session = (SessionHandler)_sessions.get( new Integer(sessionId) ) ) == null )
            throw new
    ...
    protected  class SessionHandler                {
        ...
        protected synchronized void removeUs(){
            _sessions.remove( new Integer(_sessionId ) ) ;
        }
    ...
    protected  class PrestageHandler  extends PnfsSessionHandler  {
        ...
        throws Exception {
            ...
            try{ _time = Long.parseLong( args.getOpt("stagetime" ) ) ;
            }catch(Exception ee){}
    ...
    protected  class ChmodHandler  extends PnfsSessionHandler  {
        ...
        public void fileMetaDataAvailable(){
            ...
            if( _userUid == meta.getUid() ) {
                ...
                meta.getUserPermissions().set( (_permission >> 6 ) & 0x7 ) ;
                meta.getGroupPermissions().set( (_permission >> 3 ) & 0x7 ) ;
                meta.getWorldPermissions().set( _permission  & 0x7 ) ;
    ...
    protected  class CheckFileHandler  extends PnfsSessionHandler  {
        ...
        private String _destination      = null ;
                List   _assumedLocations = null;
        private String _protocolName     = null ;
         
        private CheckFileHandler( int sessionId , int commandId , VspArgs args, List locations )
        throws Exception {
            ...
            // default try check with DCAP protocol
            ...
            try{
                ...
                if( query.getReturnCode() != 0 )
                    ...
                    query.getErrorObject() == null ? "?" :
                        ...
                        //
                        //
                        HashSet   assumedHash = new HashSet( _assumedLocations ) ;
                        List []   lists       = query.getPools() ;
                        ArrayList result      = new ArrayList() ;
                        
                        for( int i = 0 ; i < lists.length ; i++ ){
                            Iterator iterate = lists[i].iterator() ;
                            while( iterate.hasNext() ){
                                String pool = iterate.next().toString() ;
    ...
    protected  class IoHandler  extends PnfsSessionHandler  {
        ...
        throws Exception {
            ...
            if( _isHsmRequest = ( args.getOpt("hsm") != null  ) ){
        ...
        public boolean storageInfoNotAvailable() throws CacheException {
            ...
            int uid = 0 ;
            try{ uid = Integer.parseInt(_uid) ; }catch(Exception ee){}
            
            int mode = 0644;
            if( _permission != null ) {
                try { mode = Integer.parseInt(_permission, 8); } catch(Exception ee) {}
            }
        ...
        poolIoFileArrived( PoolIoFileMessage reply ){
            ...
            if( reply.getReturnCode() != 0 ){
                ...
                // bad entry in cacheInfo and pool Manager did not check it ( for performace reason )
                // try again
    ...
    protected  class OpenDirHandler  extends PnfsSessionHandler  {
        ...
        public void fileMetaDataAvailable(){
            ...
            if( ! _fileMetaData.isDirectory() ) {
                ...
                if( _ioQueueName != null )poolIoFileMessage.setIoQueueName( _ioQueueName ) ;
                if( _ioQueueAllowOverwrite &&
                    ( _ioHandlerQueue != null     ) && 
                    ( _ioHandlerQueue.length() > 0 )    )poolIoFileMessage.setIoQueueName( _ioHandlerQueue ) ;
                    
                synchronized( _messageLock ){
                    try{
                        _cell.sendMessage(  new CellMessage( new CellPath(_pool)  , poolIoFileMessage ) ) ;
                    }catch(Exception ie){
    ...
    public void   getInfo( PrintWriter pw ){
        ...
        Iterator e = _sessions.keySet().iterator() ;
        for( ; e.hasNext() ; ){
    ...
    public void   messageArrived( CellMessage msg ){
        ...
        if (object instanceof Message ){
            ...
            synchronized( _messageLock ){
                handler = (SessionHandler)_sessions.get(
                new Integer((int)reply.getId())) ;
                if( handler == null ){
