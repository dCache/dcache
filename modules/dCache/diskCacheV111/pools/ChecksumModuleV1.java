// $Id: ChecksumModuleV1.java,v 1.7 2004-07-19 14:16:10 patrick Exp $
...
public class ChecksumModuleV1  {
   ...
   private String  _checksumType = "adler32" ;
   
   private FullScan   _fullScan   = new FullScan() ;
   private SingleScan _singleScan = new SingleScan();
   ...
   public void setMoverChecksums( CacheRepositoryEntry entry ,
       ...
       //
       clientChecksum = clientChecksum == null ? 
                        ( pnfsChecksum = getChecksumFromPnfs( entry.getPnfsId() ) ) :
                        clientChecksum ;
       //
       ...
       Checksum  fileChecksum = _onWrite ? 
                                calculateFileChecksum( entry , getDefaultChecksum() ) : 
                                null;
       ...
       if( checksum == null ){
            ...
            checksum = calculateFileChecksum( entry , getDefaultChecksum() ) ;
       }
   ...
   public void storeChecksumInPnfs( PnfsId pnfsId , Checksum checksum , boolean overwrite ){
      try{
         PnfsFlagMessage flag =
            new PnfsFlagMessage(pnfsId,"c", overwrite ? "put" : "put-dont-overwrite" ) ;
         flag.setReplyRequired(false) ;
         flag.setValue(checksum.toString());
   ...
   private Checksum getChecksumFromPnfs( PnfsId pnfsId ){
     ...
     try{
         ...
         if( ( msg = _cell.sendAndWait( msg , 60000L ) ) == null )return null ;
         
         Object obj = msg.getMessageObject() ;
         if( obj instanceof PnfsFlagMessage ){
            PnfsFlagMessage flags = (PnfsFlagMessage)obj ;
            if( flags.getValue() == null ){
               _cell.esay("getChecksumFromPnfs : No crc available for "+pnfsId);
               return null ;
            }
            return new Checksum( flags.getValue().toString() ) ;
         }
     }catch(Exception ee ){
         ee.printStackTrace();
     }
   ...
   public Checksum getDefaultChecksum(){
      try{
         ...
         return checksumOf(_checksumType) ;
                              
      }catch(NoSuchAlgorithmException ae){
         return null ;
      }
   }
   private Checksum checksumOf( String type ) throws NoSuchAlgorithmException {
      return new Checksum( type.equals("adler32") ?
                           new Adler32() :
                           MessageDigest.getInstance(type) ) ;
   }
   public void dumpSetup( PrintWriter pw ){
      pw.println("csm set checksumtype "+_checksumType);
      if( _frequently ){
   ...
   public void getInfo( PrintWriter pw ){
       pw.println( "ChecksumModuleV1 : $Id: ChecksumModuleV1.java,v 1.7 2004-07-19 14:16:10 patrick Exp $" ) ;
       pw.println( "          Checksum type : "+_checksumType ) ;
   ...
   public String ac_csm_set_checksumtype_$_1( Args args ) throws Exception {
      ...
      // can we do that  ?
      //
      checksumOf( newChecksumType ) ;
      //
      // seems to be ok
      //
      _checksumType = newChecksumType ;
      return "New checksumtype : "+_checksumType ;
   }
