// $Id: MultiProtocolPool2.java,v 1.130.2.7 2007-09-16 22:39:57 patrick Exp $
...
public class       MultiProtocolPool2
    ...
    private HashMap            _moverEnvMap        = new HashMap() ;
	    ...
	    if( recover != null ){
		   ...
		   _waitForRepositoryOk = false ;
            ...
            //
            {
                ...
                for( Iterator options = _tags.entrySet().iterator() ;
                options.hasNext() ; ){
                    
                    Map.Entry e = (Map.Entry)options.next() ;
            ...
            disablePool(PoolV2Mode.DISABLED_STRICT,1,"Initiallizing");
            ...
            while( true ){
                try{
                   ...
                   disablePool(PoolV2Mode.DISABLED_STRICT,1,"Initiallizing : Repository seems not to be ready : "+ee.getMessage());
                }
                try{
                   Thread.sleep(30000) ;
    ...
    public CellVersion getCellVersion(){ return new CellVersion(diskCacheV111.util.Version.getVersion(),"$Revision: 1.130.2.7 $" ); }
    private class IoQueueManager implements JobScheduler {
        ...
        private JobScheduler getDefaultScheduler(){ return (JobScheduler)_list.get(0) ; }
        private Iterator scheduler(){
    ...
    public void getInfo( PrintWriter pw ){
        ...
        pw.println("Revision          : [$Id: MultiProtocolPool2.java,v 1.130.2.7 2007-09-16 22:39:57 patrick Exp $]" ) ;
    ...
    private class RepositoryIoHandler implements IoBatchable {
        ...
        public void run(){
            ...
            SpaceMonitorWatch spaceWatch = null ;
            try{
                ...
                if( _create ){
                    ...
                    if( ( csmover != null ) && _checksumModule.checkOnTransfer() ){
                        csmover.setDigest(_checksumModule.getDefaultChecksum());
                    }
                    ...
                    handler.runIO(
                            ...
                            _storageInfo ,
                            _pnfsId  ,
                            spaceWatch = new SpaceMonitorWatch( monitor ) ,
                ...
                try{
                    ...
                    if( _create ) {
                        ...
                        if ( (raf != null) && (raf.length() == 0) ) {
                            esay("removeing empty file: " + _pnfsId);
                            _entry.lock(false);
                            _repository.removeEntry( _entry ) ;
                ...
                _info.setTransferAttributes( bytesTransferred ,
                 ...
                 //
                 // final check for space consistency
                 //
                 if( _create ){
                     long fileSize = cacheFile.length() ;
                     try{
                        long diff     = spaceWatch.correctSpace( fileSize ) ;
                        say("Space check for "+_pnfsId+" in "+_handler.getClass().getName()+" diff "+diff+" : "+spaceWatch ) ;
                        if( diff != 0L )
                            esay("Error in space calculation for file "+_pnfsId+
                                 " with mover :"+_handler.getClass().getName()+
                                 " and difference = "+diff+" Details "+spaceWatch ) ;
                     }catch(Exception ee ){
                         esay("Exception in space calculation : "+ee ) ; 
                     }
                 }
            }
            try{
    ...
    private class CompanionFileAvailableCallback implements CacheFileAvailable {
       ...
       private CellMessage          _cellMessage = null ;
       private Pool2PoolTransferMsg _poolMessage = null ;
       private PnfsId               _pnfsId      = null ;
    ...
    private void disablePool( int mode , int errorCode , String errorString ){
        ...
        _pingThread.sendPoolDownMessage(errorCode,errorString==null?"Requested By Operator":errorString);
    ...
    private class PoolManagerPingThread implements Runnable  {
		...
		if( _poolMode.isEnabled() && _checkRepository && ! _repository.isRepositoryOk() ){
		   ...
		   disablePool( PoolV2Mode.DISABLED | PoolV2Mode.DISABLED_STRICT , 99 , "Repository got lost" ) ;
                ...
                if( _poolMode.isEnabled() )sendPoolManagerMessage(true);
                try{
        ...
        public synchronized void sendPoolDownMessage( int rc ,  String detail ){
            _poolDownMessage = new PoolManagerPoolDownMessage( _poolName , rc ,  detail ) ;
            _poolDownMessage.setPoolMode( new PoolV2Mode( _poolMode.getMode() ) );
            send( new CellMessage(
                    new CellPath( _poolManagerName ) ,
                    _poolDownMessage
        ...
        private CellMessage getPoolManagerMessage(){
            ...
            poolManagerMessage = new PoolManagerPoolUpMessage(
                    _poolName, _serialId , info );
