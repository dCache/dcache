// $Id: P2PClient.java,v 1.11.4.8 2007-10-31 22:22:45 timur Exp $
...
public class P2PClient {
   ...
   private synchronized int getNextId(){ return _nextId++ ; }
   //
   ...
   private void say( String message ){ _cell.say("PP : "+message) ; }
   private void esay( String message ){ _cell.esay("PP : "+message) ; }
   private void esay(Exception e){ _cell.esay(e); }
   public int getActiveJobs(){ return _sessions.size() <= _maxActive ? _sessions.size() : _maxActive ; }
   public int getMaxActiveJobs(){ return _maxActive ; }
   public int getQueueSize(){ return  _sessions.size() > _maxActive ? ( _sessions.size() - _maxActive ) : 0 ;  }
   //
   // the io handler
   //
   private class IOHandler implements Runnable {
      ...
      private void runIO( SpaceMonitor monitor ) throws Exception {
         ...
         // find the session id
         //
         _companion = (Companion)_sessions.get( new Integer(_sessionId) ) ;
         if( _companion == null )
            throw new
         ...
         monitor.allocateSpace(filesize) ;
         //  
      ...
      public void run(){
         try{
            ...
            SpaceMonitorWatch watch = new SpaceMonitorWatch( _repository ) ;
...
//              not having a _companion at this point means an unsolicited connect happened (e.g. a portscan)
                if ( _companion != null) {
                	long diff = watch.correctSpace( _companion.getEntry().getSize() ) ;
                	
                	
                	if( diff != 0L )esay("Space Watch error in p2p : difference = "+diff+" Details : "+watch);
                	else say("Space Watch OK for p2p : "+watch);
                }
            }
...
//          not having a _companion at this point means an unsolicited connect happened (e.g. a portscan)
            if ( _companion != null ) {
            	...
            	if( callback != null ){
            		callback.cacheFileAvailable( _companion.getEntry().getPnfsId().toString() , ioe ) ;
            	}
            }
            return ;
         ...
         CacheFileAvailable callback = _companion.getCallback() ;
         if( callback != null ){
            callback.cacheFileAvailable( _companion.getEntry().getPnfsId().toString() , null ) ;
         }
      }
      public String toString(){ return "id="+_sessionId+" "+_status ; }
   }
   public class Companion {
      ...
      private Companion( CacheRepositoryEntry entry, String srcPoolName ){ 
         _id    = getNextId() ; 
         _entry = entry ;
         _sessions.put( new Integer(_id), this ) ;
	...
	private void setCallback( CacheFileAvailable callback ){
	 ...
	 if( message.getReturnCode() != 0 ){
      ...
      private synchronized void poolTransferFinishedArrived( DoorTransferFinishedMessage message ){
	 ...
	 if( message.getReturnCode() != 0 ){
      ...
      private void remove(){
	 if( _removeOnExit )_sessions.remove( new Integer(_id) ) ;
      }
   ...
   public P2PClient( CellAdapter cell , CacheRepository repository, ChecksumModuleV1 csModule){
   ...
   public Companion newCompanion( PnfsId pnfsId , String poolName , 
      ...
      try{
         ...
         _cell.esay("Problem in sending request to "+poolName +" : "+ee.getMessage());
         _cell.esay(ee);
         _sessions.remove( new Integer( companion.getSessionId() ) ) ;
         try{
            _repository.removeEntry( entry ) ;
         }catch(Exception eee ){
   ...
   public void messageArrived( Message message , CellMessage cellMessage ){
      ...
      if( message instanceof PoolDeliverFileMessage ){
         ...
         Companion          companion = (Companion)_sessions.get(new Integer(sessionId));
         if( companion == null ){
   ...
   public void getInfo( PrintWriter pw ){
      pw.println( " Pool to Pool (P2P) [$Id: P2PClient.java,v 1.11.4.8 2007-10-31 22:22:45 timur Exp $]" ) ;
   ...
   public void printSetup( PrintWriter pw ){
      pw.println("#\n#  Pool to Pool (P2P) [$Id: P2PClient.java,v 1.11.4.8 2007-10-31 22:22:45 timur Exp $]\n#" ) ;
   ...
   public String ac_pp_remove_$_1( Args args )throws Exception {
      Object o = _sessions.remove( new Integer( args.argv(0) ) ) ;
      if( o == null )
         throw new
   ...
   public String ac_pp_keep_$_1( Args args )throws Exception {
      ...
      if( mode.equals("on") ){
        ...
        throw new
        IllegalArgumentException("Usage : pp keep on|off") ;
      }
      return "";
   }
   ...
   public String ac_pp_fail_$_1( Args args )throws Exception {
       if( args.argv(0).equals("on") ){
          ...
          throw new
          CommandSyntaxException("pp fail on|off # DEBUG ONLY");
       }
       return "Done";
   }
}
