// $Id: StorageClassInfo.java,v 1.17.4.1 2006-08-23 17:39:07 patrick Exp $ 
...
public class StorageClassInfo implements CacheFileAvailable {
   ...
   private HashMap   _failedRequests = new HashMap() ;
   ...
   private Object    _activeCounterLock = new Object() ;
   ...
   public StorageClassFlushInfo getFlushInfo(){
       ...
       synchronized( this ){
          TreeSet ts = new TreeSet( __repositoryComparator ) ;
          ts.addAll( _requests.values() ) ;
          try{
             info._time         = ts.size() == 0 ? 0L : ((CacheRepositoryEntry)ts.first()).getCreationTime() ; 
          }catch(Exception ee){
             info._time         = 0L;
          }
   ...
   public static class RepositoryEntryComparator implements Comparator {
       public int compare( Object a , Object b ){
          try{
             long al = ((CacheRepositoryEntry)a).getCreationTime() ;
             long bl = ((CacheRepositoryEntry)b).getCreationTime() ;
             return al >  bl ? 1 :
                    al == bl ? 
                    ((CacheRepositoryEntry)a).getPnfsId().toString().compareTo(((CacheRepositoryEntry)b).getPnfsId().toString())  : -1 ;
          }catch(Exception ee ){
             return -1 ;
          }
          /*
      ...
      if( ce != null ){
         ...
         if( ce instanceof CacheException ){
            ...
            if( ( cce.getRc() >= 30 ) && ( cce.getRc() < 40 ) ){
               ...
               synchronized( this){
                   CacheRepositoryEntry info = (CacheRepositoryEntry)_requests.remove( id ) ;
                   if( info != null ){
                      _time = 0L ;
...
//       }
   ...
   public synchronized void activate( PnfsId pnfsId ) throws CacheException {
      CacheRepositoryEntry info = (CacheRepositoryEntry)_failedRequests.remove( pnfsId ) ;
      if( info == null )
        throw new
   ...
   public synchronized void activateAll() throws CacheException {
      Iterator it = _failedRequests.values().iterator() ;
      while( it.hasNext() ){
         addRequest( (CacheRepositoryEntry)it.next() ) ;
      }
      _failedRequests.clear() ;
   }
   public synchronized void deactivate( PnfsId pnfsId ) throws CacheException {
      CacheRepositoryEntry info = (CacheRepositoryEntry)_requests.remove( pnfsId ) ;
      if( info == null )
        throw new
        CacheException( "Not an activated Request : "+pnfsId ) ;
        
      if( _requests.size() == 0 )_time = 0L ;
   ...
   public synchronized CacheRepositoryEntry removeRequest( PnfsId pnfsId )throws CacheException {
      ...
      if( info != null ){
         ...
         info = (CacheRepositoryEntry)_failedRequests.remove( pnfsId );
      }
   ...
   public synchronized Iterator getRequests(){ return new ArrayList(_requests.values() ).iterator() ; }
   public synchronized Iterator getFailedRequests(){ return new ArrayList( _failedRequests.values() ).iterator() ; }
   ...
   public synchronized boolean hasExpired(){
       return ( _requests.size() > 0 ) &&  
              ( ( _time + _expiration ) < new Date().getTime() ) ;
   }
   public synchronized long expiresIn(){ 
       return  _time == 0L ? 0L : (( _time + _expiration ) - new Date().getTime()  ) / 1000 ; 
   }
