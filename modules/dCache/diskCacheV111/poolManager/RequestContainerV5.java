// $Id: RequestContainerV5.java,v 1.49.2.6 2007-10-29 11:26:59 patrick Exp $ 
...
public class RequestContainerV5 implements Runnable {
    ...
    private HashMap     _handlerHash   = new HashMap() ;
    ...
    private HashMap            _selections       = new HashMap() ;
    private PartitionManager   _partitionManager = null ;
    private long               _started          = System.currentTimeMillis();    
    private long               _checkFilePingTimer = (long)(10 * 60 * 1000) ;
    private boolean            _clearSecondShot    = true ;
    ...
    public RequestContainerV5( CellAdapter cell , 
       ...
       String tmp = _cell.getArgs().getOpt("clearFailedCandidates" ) ; 
       _clearSecondShot = ! ( ( tmp != null ) && ( tmp.equals("no")  || tmp.equals("false") ) ) ;
       say("clearFailedCandidates="+_clearSecondShot);
    ...
    public void messageArrived( CellMessage cellMessage ){
       ...
       synchronized( _messageHash ){
           ...
           handler = (PoolRequestHandler)_messageHash.remove( uoid ) ;
       ...
       try{
          while( ! Thread.interrupted() ){
             ...
             try{
                ...
                PoolRequestHandler handler = null ;
                while( i.hasNext() ){
                   handler = (PoolRequestHandler)i.next() ;
       ...
       try{
          ...
          while( it.hasNext() ){
             ...
             if( rph.getPoolCandidate().equals(poolName) ||
                 rph.getPoolCandidate().equals(POOL_UNKNOWN_STRING)  ){
    ...
    public void getInfo( PrintWriter pw ){
       ...
       pw.println("Restore Controller [$Revision: 1.49.2.6 $]\n") ;
    ...
    public String ac_rc_select_$_0_3( Args args ){
       ...
       synchronized( _selections ){
          if( args.argc() == 0 ){
             ...
             Iterator i = _selections.entrySet().iterator() ;
             while( i.hasNext() ){
    ...
    public String fh_rc_retry =
       ...
       "          III) rc retry [-force-all] -p <reglar Expression>  [OPTIONS]\n\n"+
       "DESCRIPTION\n"+
       "           Forces one or more 'restore requests' to be retried.\n"+
       "           While  using syntax I , a single request  is retried,\n"+
       "           syntax II and III retries all matching requests which\n"+
       "           reported an error. Please not that a waiting request is not\n"+
       "           in an error state and consequently is not retried if wildcards\n"+
       "           are used without the -force-al option.\n"+
       "           If the '-force-all' options is given, all matching requests are\n"+
    ...
    public String ac_rc_retry_$_1( Args args ) throws CacheException {
       ...
       boolean useRegEx = args.getOpt("p")         != null ;

       if( useRegEx || args.argv(0).equals("*") ){
          ...
          Pattern  pattern = useRegEx ? Pattern.compile(args.argv(0)) : null ;
          ...
          for( Iterator it      = all.iterator() ; it.hasNext() ; ){
             ...
             try{
                ...
                if( ( pattern != null ) && ! pattern.matcher(rph.toString()).matches() )continue ;
                
                if( forceAll || ( rph._currentRc != 0 ) )rph.retry(updateSi) ;
                
             }catch(Exception ee){
                sb.append("Failed to retry :").append(rph._name).append(" : ").append(ee.getMessage()).append("\n");
             }           
          }
       }else{
    ...
    public void addRequest( CellMessage message ){
        ...
        String protocolName = protocolInfo.getProtocol()+"/"+protocolInfo.getMajorVersion() ;
        
        protocolName = _selectionUnit.getProtocolUnit( protocolName ) ;
        if( protocolName == null )
          throw new
            IllegalArgumentException("Protocol not found : "+protocolName);
        ...
        synchronized( _handlerHash ){
           //
           handler = (PoolRequestHandler)_handlerHash.get(canonicalName);
           if( handler == null ){
              _handlerHash.put( 
                     canonicalName , 
	...
	protected PnfsId       _pnfsId;
        ...
        public String getPoolCandidate(){ 
          return _poolCandidateInfo == null?
                 ( _p2pPoolCandidateInfo == null ? POOL_UNKNOWN_STRING : _p2pPoolCandidateInfo.getPoolName() ) :
        ...
        private String getPoolCandidateState(){
          return 
           ...
           _poolCandidateInfo.getPoolName() :
           _p2pPoolCandidateInfo != null ? ( ( _p2pSourcePoolInfo == null ? POOL_UNKNOWN_STRING : _p2pSourcePoolInfo.getPoolName() ) + "->" + _p2pPoolCandidateInfo.getPoolName() ) :
           POOL_UNKNOWN_STRING ;
        }
	   ...
	   return new RestoreHandlerInfo(
        ...
        private void failed( int errorNumber , String errorMessage ) 
           ...
           if( errorNumber > 0 ){
              ...
              command[0] = "failed" ;
              command[1] = new Integer(errorNumber) ;
	...
	private void sendPool2PoolRequest( String sourcePool , String destPool ) 
        ...
        private void add( Object obj ){
           ...
           synchronized( _fifo ){
               ...
               if( _threadPoolActive ){
                  _threadPool.invokeLater( new RunEngine() , "Read-"+_pnfsId ) ;
               }else{
                  _cell.getNucleus().newThread( new RunEngine() , "Read-"+_pnfsId ).start() ;
               }
           }
        }
           ...
           switch( _mode ){
              ...
              case ST_INIT :
                 ...
                 synchronized( _selections ){
                    ...
                    CacheException ce = (CacheException)_selections.get(_pnfsId) ;
                    if( ce != null ){
                 ...
                 if( inputObject == null ){
                    ...
                    if( ( rc = askIfAvailable() ) == RT_FOUND ){
                       ...
                       if( _parameter._hasHsmBackend ){
                          ...
                          _currentRc = 1010 ;
                          _currentRm = "Suspend";
                          //_poolCandidate     = POOL_UNKNOWN_STRING ;
                          _poolCandidateInfo = null ;
                          nextStep( ST_SUSPENDED , WAIT ) ;
                       }
           ...
           try{
              ...
              if( messageArrived instanceof PoolFetchFileMessage ){              
                 ...
                 if( ( _currentRc = reply.getReturnCode() ) == 0 ){
                    ...
                    // _poolCandidateInfo = (same as before);
                    return RT_OK ;
        ...
        private int askIfAvailable(){
           ...
           try{
              ...
              if(   (  _parameter._costCut     > 0.0                  ) && 
                 ...
                 if( allowFallbackOnPerformance ){
                    //
                    // if all costs are too hight, the obove list
              ...
              if( _parameter._minCostCut > 0.0 ){
                 ...
                 for( int i = 0 , n = bestAv.size() ; i < n ; i++ ){
                    ...
                    if( costValue < _parameter._minCostCut ){
                       ...
                       list.put( 
                          new Integer((_pnfsId.toString()+poolName).hashCode()) ,
                           cost ) ;
                    }
                 }
	...
	private int askForPoolToPool(boolean overwriteCost) {
            try {
                //
                //
                List sources = _pnfsFileLocation.getCostSortedAvailable();
                //
                ...
                List matrix = _pnfsFileLocation.getFetchPoolMatrix("p2p",
		                _storageInfo, _protocolInfo, null, _storageInfo
				                .getFileSize());
            ...
            //
                for (Iterator it =destinations.iterator(); it.hasNext();) {
	                PoolCheckable destinationPool = (PoolCheckable) it.next();
	                for (Iterator it2 =sources.iterator(); it2.hasNext();) {
		                if (destinationPool.getPoolName().equals( ((PoolCheckable) it2.next()).getPoolName()) ) {
			                say("removing pool "+destinationPool.getPoolName()+" from dest pool list");
			                it.remove();
		                }
	                }
                }
	            ...
	            for (Iterator i = destinations.iterator(); i.hasNext();) {
		            PoolCostCheckable cost = (PoolCostCheckable) i.next();
                ...
                for (int s = 0, sMax = sources.size(); s < sMax; s++) {
	             ...
	             PoolCheckable sourceCost = (PoolCheckable) sources.get(s);

	             String sourceHost = (String) ((map = sourceCost.getTagMap()) == null ? null
			             : map.get("hostname"));

	             for (int d = 0, dMax = destinations.size(); d < dMax; d++) {

		             PoolCheckable destinationCost = (PoolCheckable) destinations.get(d);
	             ...
	             if (sourcePool != null && destinationPool != null)break;
                }
                ...
                //   ok, we could not find a pair on different hosts, what now?
	                ...
	                if (parameter._allowSameHostCopy == PoolManagerParameter.P2P_SAME_HOST_BEST_EFFORT) {
		                ...
		                setError(137, "Not replicated : sameHostCopy=never : no matching pool found");
		                return RT_NOT_PERMITTED;

	                } else {
		                say("P2P : coding error, bad state");
		                setError(137, "Not replicated : coding error, bad state");
		                return RT_NOT_PERMITTED;
	                }
                }
        ...
        private PoolCostCheckable askForFileStoreLocation( String mode  ) throws Exception {
            ...
            if( _poolCandidateInfo == null ){
                ...
                for( Iterator i = matrix.iterator() ; i.hasNext() ; n++ ){
                 ...
                 //
                 if( ( cost == null ) && _clearSecondShot )_poolCandidateInfo = null  ;
           }
