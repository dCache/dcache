// $Id: CostModuleV1.java,v 1.16.4.2 2007-09-12 12:14:49 patrick Exp $ 
...
public class CostModuleV1 implements CostModule {
   ...
   private class Entry {
       ...
       private void setTagMap( Map map ){ _tagMap = map ; }
       private Map  getTagMap(){ return _tagMap ; }
   ...
   private class CostCheck extends PoolCheckAdapter implements PoolCostCheckable  {
       ...
       private CostCheck( String poolName , Entry e , long filesize ){
          ...
          CostCalculatable  cost = _costCalculationEngine.getCostCalculatable( e.info ) ;
   ...
   public synchronized void messageArrived( CellMessage cellMessage ){
      ...
      if( message instanceof PoolManagerPoolUpMessage ){
         ...
         PoolCostInfo.PoolQueueInfo queue = e.info.getRestoreQueue() ;
         ...
         Entry source = (Entry)_hash.get(sourceName);
         if( source == null )return ;

         PoolCostInfo.PoolQueueInfo sourceQueue = source.info.getP2pQueue() ;

         String destinationName = msg.getDestinationPoolName() ;
         Entry destination = (Entry)_hash.get(destinationName);
         if( destination == null )return ;

         PoolCostInfo.PoolQueueInfo destinationQueue = destination.info.getP2pClientQueue() ;
   ...
   public void getInfo( StringBuffer sb ){
      sb.append( "Submodule : CostModule (cm) : ").
         ...
         append("\n Version : $Id: CostModuleV1.java,v 1.16.4.2 2007-09-12 12:14:49 patrick Exp $\n") ;
   ...
   public  PoolCostCheckable getPoolCost( String poolName , long filesize ){
      ...
      Entry cost = (Entry)_hash.get(poolName);
      
      if( ( cost == null ) ||  ( _update && ! cost.isValid() )   )return null ;
   ...
   public Object ac_xcm_ls_$_0_2( Args args )throws Exception {
      ...
      if( args.argc()==0 ){   // added by nicolo : binary full cm ls list
	    ...
	    */
	   Iterator psi = _hash.keySet().iterator();
	   while(psi.hasNext()){
  		   Entry e = (Entry)_hash.get( psi.next() ) ;
  		   reply.addPoolCostInfo(e.info.getPoolName(), e.info);
	   }
	   return reply;
      ...
      if( pci ){
         ...
         Entry e = (Entry)_hash.get(poolName) ;
         Object [] reply = new Object[3] ;
         reply[0] = poolName ;
         reply[1] = e == null ? null : e.info ;
         reply[2] = e == null ? null : new Long( System.currentTimeMillis() - e.timestamp ) ;
         return reply ;
   ...
   public String hh_cm_ls = " -d  | -t | -r [<filesize>] # list all pools" ;
   public String ac_cm_ls_$_0_1( Args args )throws Exception {
      ...
      long filesize = Long.parseLong( args.argc() == 0 ? "0" : args.argv(0) ) ;
 
      while( i.hasNext() ){
         Entry e = (Entry)i.next() ;
         sb.append(e.info.toString()).append("\n") ;
         if( useReal ){ 
             PoolCostCheckable pcc = getPoolCost(e.info.getPoolName(),filesize) ; 
             if( pcc == null )
                sb.append("NONE\n") ;
             else
                sb.append( getPoolCost(e.info.getPoolName(),filesize).toString() ).
                append("\n");
         }
         if( useDetail )
             sb.append(new CostCheck(e.info.getPoolName(),e,filesize).toString()).
             append("\n");
         if( useTime )
             sb.append(e.info.getPoolName()).
                append("=").
