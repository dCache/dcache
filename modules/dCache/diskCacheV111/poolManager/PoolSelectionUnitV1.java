// $Id: PoolSelectionUnitV1.java,v 1.28 2006-05-01 20:50:50 patrick Exp $
...
public class PoolSelectionUnitV1  implements PoolSelectionUnit {

   private static final String __version = "$Id: PoolSelectionUnitV1.java,v 1.28 2006-05-01 20:50:50 patrick Exp $" ;
...
//      System.arraycopy( list.toArray() , 0 , result , 0 , result.length ) ;
   ...
   private class ProtocolUnit extends Unit {
      ...
      private ProtocolUnit( String name ){
         ...
         if( pos > -1 ){
            if( ( pos == 0 ) || ( ( name.length() - 1 ) == pos ) )
              throw new
              IllegalArgumentException("Wrong format for protocol unit <protocol>/<version>");
             
            _protocol = name.substring(0,pos);
            try{
               _version  = Integer.parseInt( name.substring(pos+1) ) ;
            }catch(Exception ee ){
               throw new
                IllegalArgumentException("Wrong format for protocol unit (version) <protocol>/<version>");
            }
         }else{
            _protocol = name ;
         }
   ...
   */
   public synchronized PoolPreferenceLevel[] 
      ...
      if( protocolUnitName != null ){
         ...
         //
         //  protocol/version not found, try protocol only
         //
         if( unit == null ){
             int tmp = protocolUnitName.indexOf('/') ;
             if( tmp > -1 )unit = (Unit)_units.get( protocolUnitName.substring(0,tmp) ) ;
         }
         //
         // not found, try to find a wildcard
         //
         if( unit == null )unit = (Unit)_units.get("*") ;
...
//      System.out.println("Matches ["+type+"] (min match count ="+fitCount+")") ;
      while( links.hasNext() ){
         Link link = (Link)links.next() ;
...
//         links = sortedSet.iterator() ;
//         while( links.hasNext() )System.out.println("   "+links.next() ) ;
   ...
   public String getProtocolUnit( String protocolUnitName ){
      throw new
         IllegalArgumentException("PoolSelection unit 1 not longer supported");
   }
   ...
   public String hh_psux_ls_link = "[<link>]" ;
   ...
   public String hh_psu_set_link = 
      "<link> [-readpref=<readpref>] [-writepref=<writepref>] "+
             "[-cachepref=<cachepref>] [-p2ppref=<p2ppref>] [-section=<tag>|NONE]" ;
