// $Id: HttpPoolMgrEngineV3.java,v 1.22 2006-05-06 19:02:39 patrick Exp $
...
public class HttpPoolMgrEngineV3 implements HttpResponseEngine, Runnable {
   ...
   private ArrayList   _lazyRestoreList = new ArrayList() ;
   private Object      _lazyRestoreLock = new Object() ;
   ...
   private void decodeDetails( String details ){
       if( ( ! details.startsWith("details=") ) || ( details.length() < 9 ) )return ;
       StringTokenizer st = new StringTokenizer( details.substring(8) ,"," ) ;
       ArrayList list = new ArrayList() ;
       while( st.hasMoreTokens() )list.add(st.nextToken()) ;
       _siDetails = (String [])list.toArray( new String[0]) ;
   }
   public void run(){
      ...
      while( ! Thread.currentThread().interrupted() ){
         try{
            ...
            _nucleus.esay( "Restore Collector got : "+ee ) ;
            _nucleus.esay(ee);
         }
      }
   }
   public void getInfo( PrintWriter pw ){
      pw.println(" PoolManagerEngine : [$Id: HttpPoolMgrEngineV3.java,v 1.22 2006-05-06 19:02:39 patrick Exp $]" ) ;
   ...
   private void runRestoreCollector() throws Exception {
      ...
      ArrayList agedList = new ArrayList() ;
      Object [] list     = (Object [])o ;
      long      cut      = System.currentTimeMillis() - ( 1000L * 60L * 2L ) ;
      for( int i = 0 ; i < list.length ; i++ ){
         RestoreHandlerInfo info = (RestoreHandlerInfo)list[i] ;
      ...
      synchronized( _lazyRestoreLock ){ _lazyRestoreList = agedList ; }
   }
             ...
             if(  urlItems.length > 2 ){
                 ...
                 if( urlItems[2].equals("lazy")  ){
                      ...
                      if( ( urlItems.length > 3 ) &&  urlItems[3].startsWith("detail") ){
                          Map map = createMap( urlItems[3] ) ;
                          String sort = (String)map.get("sort");
                          String grep = (String)map.get("grep");
                          printMenu( pw ,  sort , grep  ) ;
                          printLazyRestoreInfo( pw , sort , grep ) ;
                      }else{
                      //
                      // regular restore queue
                      //
                          printLazyRestoreInfo( pw , null ,null ) ;
                      }
                 }else if( urlItems[2].startsWith("detail") ){
                       Map map = createMap( urlItems[2] ) ;
                       String sort = (String)map.get("sort");
                       String grep = (String)map.get("grep");
                       printMenu( pw , sort , grep ) ;
                       printRestoreInfo( pw , sort , grep ) ;
                 }else{
                     printRestoreInfo( pw ,  null , null ) ;        
                 }
             }else{
	        printRestoreInfo( pw ,  null , null ) ;
             }
   ...
   private void printParameter( PrintWriter pw , String key ){
      ...
      pw.println( "<hr><address>Created "+( new Date() )+" $Id: HttpPoolMgrEngineV3.java,v 1.22 2006-05-06 19:02:39 patrick Exp $");
   ...
   private void printParameterInMatrix( PrintWriter pw , Map parameterMap ){
       ...
       for( Iterator n = defaultMap.entrySet().iterator() ; n.hasNext() ; row++){
       
           Map.Entry entry = (Map.Entry)n.next() ;
           String key      = (String)entry.getKey() ;
           Object [] e     = (Object [])entry.getValue() ;
           boolean isSet   = ((Boolean)e[0]).booleanValue() ;
           ...
           for( int m = 0 ; m < restMap.length ; m++ ){
               ...
               isSet = ((Boolean)e[0]).booleanValue() ;
               value = e[1].toString() ;
   ...
   private void printParameterEntry( PrintWriter pw , PoolManagerParameter p ){
      ...
      for( Iterator i = map.entrySet().iterator() ; i.hasNext() ; column = ( column + 1 ) % ( maxColumn+1 ) ){
      
         Map.Entry  entry = (Map.Entry)i.next() ;
         String     name  = entry.getKey().toString();
         Object []  array = (Object [])entry.getValue() ;
         boolean    isSet = ((Boolean)array[0]).booleanValue() ;
   ...
   private void printLazyRestoreInfo( PrintWriter pw , String sorting , String grep  ){
   
      printRestoreHeader(pw, "dCache Dataset Restore Monitor (Lazy)");
//      pw.println("<br>");
//      printMenu( pw ,  sorting , grep  ) ;
//      pw.println("<br>");
      printRestoreTableHeader(pw);
      
      ArrayList agedList = null ;
      synchronized( _lazyRestoreLock ){ agedList = _lazyRestoreList ; } ;
      
      ArrayList copy = new ArrayList( agedList ) ;
      try{
   ...
   private void printRestoreInfo( PrintWriter pw , String sorting , String grep ){
   
      printRestoreHeader(pw, "dCache Dataset Restore Monitor");
      ...
      try{
         ...
         if( o instanceof Exception ){
            ...
            for( int i = 0 , j = 0 ; i < list.length ; i++ ){
               try{
                  if( ( grep == null ) || grepOk(grep,list[i]) )
                        showRestoreInfo( pw , (RestoreHandlerInfo) list[i] , j++) ;
               }catch(Exception ioe){}
            }
            pw.println("</table></center>");
         }
...
//           System.out.println("Comparator !!!! iniitialized with "+type);
       ...
       private int compareInfo( RestoreHandlerInfo i1 , RestoreHandlerInfo i2 ){
           if( _type == null ){
               ...
               return new Long( i1.getStartTime() ).compareTo( new Long( i2.getStartTime() ) ) ;
           }else{
               return i1.getName().compareTo(i2.getName());
           }
       }
   ...
   private void showRestoreInfo( PrintWriter pw , RestoreHandlerInfo info , int row ){
      showRestoreInfo( pw , info , null , null , row ) ;
   }
   private void printRestoreHeader( PrintWriter pw , String title ){
      pw.println( "<html>");
      ...
      pw.println( "<h1><font color=blue>"+title+"</font></h1>");
      pw.println( "</td></tr></table>");
   }
   private void printRestoreTableHeader( PrintWriter pw ){
      String prefix = "<th bgcolor=\"#115259\"><font color=white>" ;
      pw.println("<center><table  border=1 cellspacing=0 cellpadding=5 width=\"90%\">");
      pw.println("<tr>");
      pw.println(prefix+"PnfsId</font></th>") ;
      pw.println(prefix+"Subnet</font></th>");
      pw.println(prefix+"PoolCandidate</font></th>");
      pw.println(prefix+"Started</font></th>");
      pw.println(prefix+"Clients</font></th>");
      pw.println(prefix+"Retries</font></th>");
      pw.println(prefix+"Status</font></th></tr>");
   }
   private void showRestoreInfo( PrintWriter pw , 
     ...
     String  prefix = "<td align=center bgcolor=\""+
                      background[row%background.length]+
     ...
     pw.println("<tr>");
     pw.println(prefix+pnfsId+postfix);
     pw.println(prefix+subnet+postfix);
     pw.println(prefix+pool+postfix);
     pw.println(prefix+started+postfix);
     pw.println(prefix+info.getClientCount()+postfix);
     pw.println(prefix+info.getRetryCount()+postfix);
     pw.println(prefix+status+postfix);
     pw.println("</tr>");
     if( path != null ){
        pw.println("<tr><td colspan=7 align=center bgcolor=\""+
                   background[row%background.length]+"\"><font color=\""+foreground+"\">");
        pw.println(path) ;
     ...
     if( storageInfo != null ){
        ...
        if( _siDetails != null ){
            ...
            for( int i = 0 ; i < _siDetails.length  ; i++ ){
               key = _siDetails[i] ;
               
               if( ( value = storageInfo.getKey(key) ) == null )continue ;
               
               pw.print(key+"="+value+";");
            }
            pw.println("");
        }else{   
            pw.println(storageInfo.toString()) ;
        }
        pw.println("</font></td></tr>");
     }
     if( error ){
        pw.println("<tr><td colspan=7 bgcolor=\""+
                   __errorBackground[row%__errorBackground.length]+"\">");
        pw.println("<font color=white> Code = "+rc+" ; Message = "+msg+"</font>" ) ;
        pw.println("</td></tr>");
     }
   }
   ...
   private void showMatch( PrintWriter pw , String message ) throws Exception {
      Map map = createMap( message ) ;
      String type   = (String)map.get("type") ;
      String store  = (String)map.get("store") ;
      String dcache = (String)map.get("dcache");
      String net    = (String)map.get("net");
      String prot   = (String)map.get("protocol");
      store  = ( store  == null ) || ( store.equals("")  ) ? "*" : store ;
      dcache = ( dcache == null ) || ( dcache.equals("") ) ? "*" : dcache ;
      net    = ( net    == null ) || ( net.equals("")    ) ? "*" : net ;
      prot   = ( prot   == null ) || ( prot.equals("")   ) ? "*" : prot ;
      pw.println("<center>");
      if( type == null ){
   ...
   private void queryAll(PrintWriter pw , String request , String type ) throws Exception {
      CellMessage reply = _nucleus.sendAndWait( 
                                new CellMessage( 
                                      new CellPath("PoolManager" ) ,
                                      request 
   ...
   private void queryPool( PrintWriter pw , String poolName )throws Exception {
      ...
      boolean   enabled   = ((Boolean)o[3]).booleanValue() ;
      boolean   rdOnly    = ((Boolean)o[5]).booleanValue() ;
      long      active    = ((Long)o[4]).longValue() ;
   ...
   private void queryLinkList( PrintWriter pw )throws Exception {
      ...
      List inlist = (List)answer ;
      List list   = new ArrayList() ;
      for( Iterator i = inlist.iterator() ; i.hasNext() ; ){
          list.add( new LinkProperties( (Object [])i.next() ) );
      }
      ...
      for( Iterator i = list.iterator() ; i.hasNext() ; row++  ){
          LinkProperties lp = (LinkProperties)i.next() ;
   ...
   private void queryUnitGroup( PrintWriter pw , String groupName )throws Exception {
   ...
   private void queryPoolGroup( PrintWriter pw , String groupName )throws Exception {
