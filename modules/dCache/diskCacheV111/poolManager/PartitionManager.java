// $Id: PartitionManager.java,v 1.2.2.3 2007-05-27 19:23:25 patrick Exp $
...
public class PartitionManager implements java.io.Serializable {
    ...
    public class Info implements java.io.Serializable  {
        ...
        private PoolManagerParameter _parameter = null ;
    ...
    private Info        _defaultPartitionInfo = null ; 
    ...
    public PartitionManager( CellAdapter adapter ){
       ...
       _infoMap.put( "default" , _defaultPartitionInfo  = new Info("default") ) ;
    ...
    public Map getParameterMap(){
       ...
       for( Iterator i = _infoMap.entrySet().iterator() ; i.hasNext() ; ){
    ...
    public PoolManagerParameter getParameterCopyOf( String partitionName ){
       ...
       Info info = (Info)_infoMap.get( partitionName ) ;
    ...
    public Info getInfoPartitionByName( String partitionName ){
        ...
        return (Info)_infoMap.get( partitionName ) ;
    ...
    public String ac_pm_set_$_0_1( Args args ){
        ...
        Info info = (Info)_infoMap.get(name) ;
    ...
    public String ac_pm_ls_$_0_1( Args args ){
       ...
       if( args.argc() == 0 ){
          if( extended ){
              ...
              for( Iterator it = _infoMap.keySet().iterator() ; it.hasNext() ; ){
                  String sectionName = it.next().toString() ;
                  sb.append(sectionName).append("\n");
              }
          }
       }else{
          String sectionName = args.argv(0) ;
          Info info = (Info)_infoMap.get(sectionName) ;
          if( info == null )
             throw new
    ...
    public void dumpSetup( StringBuffer sb ){
        ...
        if( _useLegacySetupPrintout ){
           ...
           Info info = (Info)_infoMap.get("default");
           dumpInfo( sb , info ) ;
        }
        for( Iterator i = _infoMap.values().iterator() ; i.hasNext() ; ){
    ...
    private void dumpInfo( StringBuffer sb , Info info ){
       ...
       dumpCostOptions( sb , info._name , para ) ;
         
       dumpThresholdOptions( sb , info._name , para ) ;
       
       dumpP2pOptions( sb , info._name , para ) ;
       
       dumpStageOptions( sb , info._name , para ) ;
       
       dumpMiscOptions( sb , info._name , para ) ;
       
    }
    private void dumpCostOptions( StringBuffer sb , String name , PoolManagerParameter para ){
       if( para._performanceCostFactorSet )
          sb.append("pm set ").append(name).append(" -cpucostfactor=").append(para._performanceCostFactor).append("\n") ;
       if( para._spaceCostFactorSet )
          sb.append("pm set ").append(name).append(" -spacecostfactor=").append(para._spaceCostFactor).append("\n") ; 
    }
    private void dumpThresholdOptions( StringBuffer sb , String name , PoolManagerParameter para ){
       if( para._minCostCutSet )
         sb.append("pm set ").append(name).append(" -idle=").append(para._minCostCut).append("\n") ; 
       if( para._costCutSet )
         sb.append("pm set ").append(name).append(" -p2p=").append(para._costCut).append("\n") ; 
       if( para._alertCostCutSet )
         sb.append("pm set ").append(name).append(" -alert=").append(para._alertCostCut).append("\n") ; 
       if( para._panicCostCutSet )
         sb.append("pm set ").append(name).append(" -halt=").append(para._panicCostCut).append("\n") ; 
       if( para._fallbackCostCutSet )
         sb.append("pm set ").append(name).append(" -fallback=").append(para._fallbackCostCut).append("\n") ; 
    }
    private void dumpP2pOptions( StringBuffer sb , String name ,  PoolManagerParameter para ){
       if( ! para._p2pAllowed ){
           if( para._p2pAllowedSet )sb.append("pm set ").append(name).append(" -p2p-allowed=false").append("\n") ;
       }else{
           if( para._p2pAllowedSet )sb.append("pm set ").append(name).append(" -p2p-allowed").append("\n") ;
           if( para._p2pOnCostSet && para._p2pOnCost )
               sb.append("pm set ").append(name).append(" -p2p-oncost").append("\n") ;
           if( para._p2pForTransferSet && para._p2pForTransfer )
               sb.append("pm set ").append(name).append(" -p2p-fortransfer").append("\n");
       }
    }
    private void dumpStageOptions( StringBuffer sb , String name , PoolManagerParameter para ){
       if( ! para._hasHsmBackend ){
           if( para._hasHsmBackendSet )
              sb.append("pm set ").append(name).append(" -stage-allowed=false").append("\n")  ;
       }else{
           if( para._hasHsmBackendSet )
              sb.append("pm set ").append(name).append(" -stage-allowed").append("\n")  ;
           if( para._stageOnCostSet && para._stageOnCost )
              sb.append("pm set ").append(name).append(" -stage-oncost").append("\n")  ;
       }
    }
    private void dumpMiscOptions( StringBuffer sb , String name , PoolManagerParameter para ){
       if( para._maxPnfsFileCopiesSet )
           sb.append("pm set ").append(name).append(" -max-copies=").append(para._maxPnfsFileCopies).append("\n") ;
    }
    /**
      *   legacy output 
      */
    private void dumpLegacyDefaultInfo( StringBuffer sb , Info info ){
       ...
       sb.append("set pool decision").
          append(" -cpucostfactor=").append(para._performanceCostFactor).
          append(" -spacecostfactor=").append(para._spaceCostFactor).
          append("\n") ; 
         
       sb.append("set costcuts").
         append(" -idle=").append(para._minCostCut).
