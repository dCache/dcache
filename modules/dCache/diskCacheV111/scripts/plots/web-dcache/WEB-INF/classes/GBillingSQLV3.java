/*
  ...
  Revision 1.1  2003/09/11 22:51:17  cvs
  New plot for connection time has been added

  Revision 1.5  2003/06/18 20:39:21  cvs
  Nullify closed streams for better garbage collection.

  Revision 1.4  2003/06/09 21:23:50  cvs
  Fix memory leak during plot building with gnuplot

  Revision 1.3  2003/05/21 16:31:50  cvs
  Fix bug for cost_daily and hits_daily tables

  Revision 1.2  2003/05/15 21:35:46  cvs
  Change the color for log plots to red

  Revision 1.1  2003/05/14 23:19:17  cvs
  Quick hack to add current week plots

  Revision 1.6  2003/05/13 15:35:05  cvs
  Add cache hit and cost plots
...
public class GBillingSQLV3 extends HttpServlet implements Runnable
{
	...
	log("Call buildYearPlots() from init");
	try {
	    buildYearPlots();
	    buildMonthlyPlots();
	    buildWeekPlots();
...
//                         stdOutput.println("plot '-' using 1:2:3:4:(-1) t '"+title1+"' with boxerrorbars lw 4 1 ");
                        stdOutput.println("plot '-' using 1:2 t '"+title1+"' with linespoints lw 2 1, " +
                                          ...
                                          "'-' using 1:2 t 'Max' with linespoints lw 2 3 "
                                          );
                        for (int ib = 0; ib < x1.length; ib++) {
                            ix = x1[ib];    iy = y1[ib];
                            nEnt++;
    ...
    /**
     * This is the method run by the update thread, it just calls buildYearPlots
     * once an hour.
     */
    public void run()
    {
    ...
    */
	...
	try	{
		...
		stmt.executeUpdate("select date(datestamp), avg(connectiontime)/1000 as avg, min(connectiontime)/1000 as min, max(connectiontime)/1000 as max, stddev(connectiontime)/1000 as stddev, count(*) " +
	    ...
	    // New table: connection time
	    stmt.executeUpdate("delete from dc_tm_daily where date>='"+weekAgo+"'");
	    log("delete from dc_tm_daily where date>='"+weekAgo+"'");
            stmt.executeUpdate("insert into dc_tm_daily " +
                               "select date(datestamp), avg(connectiontime)/1000 as avg, min(connectiontime)/1000 as min, max(connectiontime)/1000 as max, stddev(connectiontime)/1000 as stddev, count(*) " +
                               "from billinginfo " +
			       "where datestamp>='"+weekAgo+"' and errorcode=0 group by date(datestamp)");
	}
	catch (SQLException x) {
	...
	try	{
	    ...
	    // Process writes to dCache
	    log("Process dc_wr_hourly");
	    DataSet3 dc_wr = new DataSet3(con, "dc_wr_hourly", "datestamp >= '"+sDate+"' and datestamp < '"+eDate+"'");
...
//             stdOutput.println("plot '-' using 1:2:3:4:(-1) t '"+title1+"' with boxerrorbars lw 2 1");
	...
	try {
	    ...
	    stdOutput.println("set terminal postscript eps color solid 'Arial' 24");
	    stdOutput.println("set xlabel '"+xlabel+"'");
	    stdOutput.println("set timefmt '%Y-%m-%d-%H'");
	    stdOutput.println("set xdata time");
	    stdOutput.println("set xrange ['"+(sDate.getYear()+1900)+"-"+(sDate.getMonth()+1)+"-"+sDate.getDate()+"':'"+
			      (eDate.getYear()+1900)+"-"+(eDate.getMonth()+1)+"-"+eDate.getDate()+"']");
	    log("set xrange ['"+(sDate.getYear()+1900)+"-"+(sDate.getMonth()+1)+"-"+sDate.getDate()+"':'"+
		(eDate.getYear()+1900)+"-"+(eDate.getMonth()+1)+"-"+eDate.getDate()+"']");
	    stdOutput.println("set grid");
	    if (pltFmt.startsWith("log")) {
		stdOutput.println("set log y");
		...
		for (int ib = 0; ib < x1.length; ib++) {
		    ix = x1[ib];    iy = y1[ib];
		    nEnt++;    totl += iy;
		    stdOutput.println((ix.getYear()+1900)+"-"+(ix.getMonth()+1)+"-"+ix.getDate()+"-"+ix.getHours()+" "+iy);
		    log("X1 "+ix+": "+iy);
		}
		stdOutput.println("e");
		log("e");
		...
		for (int ib = 0; ib < x1.length; ib++) {
		    ix = x1[ib];    iy = y1[ib];    ie = y2[ib];
		    nEnt++;    totl += iy;
		    stdOutput.println((ix.getYear()+1900)+"-"+(ix.getMonth()+1)+"-"+ix.getDate()+"-"+ix.getHours()+" "+iy+ " "+ie);
		    log("X "+ix+": "+iy+" "+ie);
		}
	    }
	    stdOutput.println("e");
...
//                 log              ((ix.getYear()+1900)+"-"+(ix.getMonth()+1)+"-"+ix.getDate()+"-"+ix.getHours()+" "+iy+" "+il+" "+ih);
//             }
...
// 	    log("e");
            ...
            for (int ib = 0; ib < x1.length; ib++) {
                ...
                nEnt++;
                stdOutput.println((ix.getYear()+1900)+"-"+(ix.getMonth()+1)+"-"+ix.getDate()+"-"+ix.getHours()+" "+iy);
                log              ((ix.getYear()+1900)+"-"+(ix.getMonth()+1)+"-"+ix.getDate()+"-"+ix.getHours()+" "+iy);
            }
	    stdOutput.println("e");
	    log("e");
