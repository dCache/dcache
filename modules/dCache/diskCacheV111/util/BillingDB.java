//
// $Id: BillingDB.java,v 1.31.2.1 2006-09-19 17:04:24 podstvkv Exp $
//
...
//VP
        ...
        if( _maxTimeBeforeCommit > 0 ) {
            new TimeCommiter(_con, _maxTimeBeforeCommit).start();
        }
    }
...
//VP
    ...
    private void shrinkDB( long upTo ) {
        ...
        for( int i = 0; i < _tableList.length; i++) {
            ...
            String sqlQuery = "DELETE FROM "+ _tableList + " WHERE dateStamp < ?";            
            try {
    ...
    class TimeCommiter extends Thread {
        ...
        TimeCommiter( Connection c, int t) {
            sqlCon = c;
            timeout = t;
        }
        ...
        public void run() {
            while (true) {
                try {
                    System.err.println("TimeCommiter: COMMIT: "+timeout);
//                  wait(timeout);
