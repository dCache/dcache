// $Id: PinManager.java,v 1.31.2.6 2007-10-31 22:23:26 timur Exp $
// $Log: not supported by cvs2svn $
// Revision 1.31.2.5  2007/08/22 16:13:34  timur
// PinManager timeouts are configurable, updateWaitQueue thread detects message timeouts
//
// Revision 1.31.2.4  2007/07/16 22:03:14  timur
// Revision 1.40  2007/08/15 21:43:56  timur
// added thread for cell message send with lifetime expiration notification
//
...
//
// Revision 1.31.2.3  2007/04/18 23:33:22  timur
// initial state of the pin should be INITIAL_STATE and not PINNED\!
//
// Revision 1.31.2.2  2006/11/30 13:52:56  tigran
// message "stopTimer(): timer not found for requestId="
// not an error message any more
//
// Revision 1.31.2.1  2006/09/19 17:03:25  podstvkv
...
public class PinManager extends CellAdapter implements Runnable  {
    ...
    private long pnfsManagerTimeout = 10*60*1000L; //ten minutes
    private long poolManagerTimeout = 60*60*1000L; //one hour
    private long poolTimeout = 20*60*1000L;         //twenty minutes
    
    //keep the names  spelled in the lower case to make postgress driver to work
    // correctly
    
    private String PinRequestTableName = "pinrequestsv1";
    private String NextPinRequestIdTableName = "nextpinrequestid";
    private String OldPinRequestTableName = "pinrequests";
    private String OldPinsTableName = "pins";
    ...
    private Map pinTimerTasks = new Hashtable();
    ...
    private Map pnfsIdToPins = new Hashtable();
    ...
    private List databaseStatementsQueue = new LinkedList();
    // database types
    ...
    public PinManager(String name , String argString) throws Exception {
        ...
        if(_args.getOpt("maxPinDuration") != null) {
         ...
         String tmpstr = _args.getOpt("pnfs-timeout");
         if(tmpstr != null) {
         ...
         tmpstr = _args.getOpt("pool-timeout");
         if(tmpstr != null) {
    ...
    public CellVersion getCellVersion(){ return new CellVersion(diskCacheV111.util.Version.getVersion(),"$Revision: 1.31.2.6 $" ); }

    public long nextPinrequestId() {
        synchronized(databaseStatementsQueue) {
            nextRequestId++;
            databaseStatementsQueue.add("UPDATE "+NextPinRequestIdTableName+
            " SET NEXTLONG="+nextRequestId);
    ...
    public String ac_pin_pnfsid_$_2( Args args ) throws Exception {
        ...
        lifetime *=1000;
        Object o = pin(pnfsId,lifetime,null,null);
        if(o instanceof Long) {
            return "pin successfull, pin id:"+o;
        }
        
        Pinner pinner = (Pinner) (((Object[])o)[1]);
        Long   pinId = (Long) (((Object[])o)[0]);
        pinner.waitCompleteon();
        if(pinner.isSuccess()) {
            return "pin succefull, pin id :"+pinId;
        } else {
            return "pin failed, reason :"+  pinner.getErrorObject();          
        }
    }
    ...
    public String ac_unpin_$_2( Args args ) throws Exception {
        ...
        Object o = unpin(pnfsId, pinRequestId,null,null);
        if(o instanceof Unpinner) {
            Unpinner unpinner =  (Unpinner ) o;
            unpinner.waitCompleteon();
            if(unpinner.isSuccess()) {
                return " unpinning Successfull";
            }

            return "unpinning failed: "+unpinner.getErrorObject();
        }
    ...
    public String ac_ls_$_0_1( Args args ) throws Exception {
        if(args.argc() >0) {
            PnfsId pnfsId = new PnfsId(args.argv(0));
            Pin pin = (Pin) pnfsIdToPins.get(pnfsId);
            return pin.toString();
        }
        StringBuffer sb = new StringBuffer();
        Set pnfsIds = pnfsIdToPins.keySet();
        if(pnfsIds.isEmpty()) {
            return "no files are pinned";
        }
        for(Iterator i = pnfsIds.iterator(); i.hasNext();) {
            PnfsId pnfsId = (PnfsId)i.next();
            Pin pin = (Pin) pnfsIdToPins.get(pnfsId);
            pin.append(sb);
        }
                ...
                if(tableRs.next()) {
                    ...
                    OldPinRequestTableName+".PnfsId, "+
                     ...
                     while(rs.next()) {
                        ...
                        pinRequestId+",\'"+pnfsId+"\',"+expiration+")");
    ...
    private void initializeDatabasePinRequests() throws SQLException {
        ...
        try {
            ...
            while(rs.next()) {
                ...
                PinRequest pinRequest = new PinRequest(pinRequestId, expiration);
    ...
    public void processMessage( CellMessage cellMessage ) {
        ...
        if(o instanceof PinManagerUnpinMessage) {
    ...
    public String pin(PnfsId pnfsId,long lifetime,PinManagerPinMessage pinRequest,CellMessage cellMessage) {
        ...
        synchronized(pnfsIdToPins) {
            ...
            PinRequest _pinRequest = 
               new PinRequest(pinRequestId, System.currentTimeMillis()+lifetime);
    ...
    public void startTimer(final PnfsId pnfsId, final long requestId,long lifetime) {
        TimerTask tt = new TimerTask() {
            public void run() {
                ...
                Object o = pinTimerTasks.remove(new Long(requestId));
                if(o == null) {
        ...
        pinTimerTasks.put(new Long(requestId), tt);
    ...
    public void stopTimer( long requestId) {
        final Long requestIdLong = new Long(requestId);
        Object o = pinTimerTasks.remove(requestIdLong);
        if(o == null) {
        	// is it really an error ?
            say("stopTimer(): timer not found for requestId="+requestId);
            return;
        }
        ...
        if(Thread.currentThread() == this.databaseUpdateThread) {
            while(true) {
                ...
                synchronized(databaseStatementsQueue) {
                    ...
                    nextSqlStatement = (String) databaseStatementsQueue.remove(0);
    ...
    public  class PinRequest  {
        ...
        public PinRequest(long pinRequestId,long expiration) {
        ...
        public void answerArrived(CellMessage question, CellMessage answer) {
            ...
            if(o instanceof Message) {
                ...
                if(message instanceof PnfsFlagMessage) {
                    ...
                    java.util.Vector locations = cachedLocationsResponce.getCacheLocations();
                    for(java.util.Iterator i = locations.iterator();
                    i.hasNext();) {
                        String nextLocation = (String)i.next();
