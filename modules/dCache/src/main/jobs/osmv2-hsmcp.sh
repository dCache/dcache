...
#set -x
#
if [ ! -f /etc/dcache.conf ] ; then
   echo "dcache.conf not found" >&2
   exit 3
fi
#
. /etc/dcache.conf
#
OSMCP=${DCACHE_HOME}/jobs/osmcp
DCCP=${DCACHE_HOME}/jobs/dccp
LOG=/tmp/osmcp.log
DEVTTY=$LOG
#DEVTTY=/dev/tty
#
...
#
   if [ \( -z "${store}" \) -o \( -z "${group}" \) -o \( -z "${bfid}" \) ] ; then
      echo "couldn't get sufficient info for 'osmcp' : store=>${store}< group=>${group}< bfid=>${bfid}<" 1>&2
      exit 14 
   fi 
   if [ "${hsmProvider}" = d2cache ] ; then
      [ -z "${d2server}" ] && d2server=d2cachedoor:24125
      echo "$DCCP -d2 pnfs://${d2server}/${pnfsid} ${filename}"
      $DCCP -d2 pnfs://${d2server}/${pnfsid} ${filename} 1>>${LOG} 2>&1
      osm_error=$?
   else
