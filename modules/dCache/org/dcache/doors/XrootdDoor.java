...
//	is set true only once (when plugin loading didn't succeed) to avoid multiple trials
	...
	private final static int XROOD_PROTOCOL_MINOR_VERSION = 4;
...
//		at this point, we can't determine, which of the pool IP-addresses is the right one, so we select the first
		...
		if (redirectAddress != null) {
			_redirectTable.put(new Integer(reply.getXrootdFileHandle()), redirectAddress);
		} else {
...
//			we have to put a null address to at least notify the right xrootd thread about the failure 
			_redirectTable.put(new Integer(reply.getXrootdFileHandle()), "");
		}
	}
...
//				general error in pool, cannot recover (request must fail)
		...
		Integer key = new Integer( protocolInfo.getXrootdFileHandle() );
...
//			
//			Integer fileHandle = new Integer(((XrootdProtocolInfo) finishedMsg.getProtocolInfo()).getXrootdFileHandle());
//			
...
//			}
	...
	public void forgetFile(int fileHandle) {
		_logicalStreamTable.remove(new Integer(fileHandle));		
	}
	...
	public void newFileOpen(int fileHandle, int streamID) {
		_logicalStreamTable.put(new Integer(fileHandle), new Integer(streamID));
	}
