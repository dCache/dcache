...
import org.dcache.pool.repository.v3.PartialFromClientException;
import org.dcache.pool.repository.v3.PartialFromStoreException;
import org.dcache.pool.repository.v3.RepositoryException;
import org.dcache.pool.repository.v3.RepositoryMissmatchException;
...
public class CacheRepositoryEntryV3Impl implements CacheRepositoryEntry {
	...
	public CacheRepositoryEntryV3Impl(CacheRepositoryV3 repository, PnfsId pnfsId, File controlFile, File dataFile, File siFile ) throws IOException, RepositoryException {
		...
		if( _state.isRecivingFromClient() ) {
			throw new PartialFromClientException("partialy recived from client file");
		}

		if( _state.isRecivingFromStore() ) {
			throw new PartialFromStoreException("partialy recived from store file");
		}
		...
		if( _storageInfo != null && _storageInfo.getFileSize() != _dataFile.length() ) {
			throw new RepositoryMissmatchException("data file size(" + dataFile.length() + ") does not match to file size in storageInfo ("+ _storageInfo.getFileSize()+")");
		}
	...
	public void setCached() throws CacheException {
		try {
			...
			if( _state.isRecivingFromClient() || _state.isRecivingFromStore() ) {
				CacheRepositoryEvent availableEvent = new CacheRepositoryEvent(_repository, clone() );
				_repository.processEvent(EventType.AVAILABLE, availableEvent);
			}

			CacheRepositoryEvent cachedEvent = new CacheRepositoryEvent(_repository, clone() );
			_repository.processEvent(EventType.CACHED, cachedEvent);
	...
	public void setPrecious(boolean force) throws CacheException {
		try {
			...
			CacheRepositoryEvent preciousEvent = new CacheRepositoryEvent(_repository, clone() );
			_repository.processEvent(EventType.PRECIOUS, preciousEvent);
	...
	public void setReceivingFromClient() throws CacheException {
		try {
			...
			CacheRepositoryEvent createEvent = new CacheRepositoryEvent(_repository, clone() );
			_repository.processEvent(EventType.CREATE, createEvent);
	...
	public void setSticky(boolean sticky) throws CacheException {
		try {
			...
			CacheRepositoryEvent sickyEvent = new CacheRepositoryEvent(_repository, clone() );
			_repository.processEvent(EventType.STICKY, sickyEvent);
