 ...
 * weighted randomly destination pool. P2p transfers never use the same host/tag
 * as source and destination. The weight of random selection calculated as:
 *
 * pools free space / total free space
 *
 */
public class WRandom extends Partition {
    ...
    private int selectWrandomIndex(WeightedPool[] weghtedPools) {
        double selection = _random.nextDouble();
        double total = 0;
        int i = 0;
        for (i = 0; (i < weghtedPools.length) && (total <= selection); i++) {
            total += weghtedPools[i].getWeight();
        }
        return i - 1;
    }
    ...
    private class DifferentHost implements Predicate<PoolInfo> {
        ...
        public boolean apply(PoolInfo t) {
            ...
            return !(_host.hashCode() == hostname.hashCode()
                    && _host.equals(hostname));
        }
    }
}
