...
public class Rebalancer
    ...
    private ListenableFuture<List<PoolMigrationJobCancelMessage>> cancelAll(
          Collection<SelectionPool> pools, String why) {
        return allAsList(pools.stream()
              .map(pool -> new CellPath(pool.getName()))
              .map(path -> CellStub.transform(
                    _poolStub.send(path, new PoolMigrationJobCancelMessage(JOB_NAME, true, why)),
                    m -> m))
              .collect(toList()));
    }

    private ListenableFuture<List<String>> sendToAll(Collection<SelectionPool> pools,
          String command) {
        return allAsList(pools.stream()
              .map(pool -> new CellPath(pool.getName()))
        ...
        public Reply call() throws NoSuchElementException, IllegalArgumentException {
            ...
            switch (metric) {
                case METRIC_RELATIVE:
                    ...
                    command =
                          String.format(Locale.US,
                                "migration move -id=%s -include-when='target.used " +
                ...
                case METRIC_FREE_COST:
                    command =
                          String.format(Locale.US,
                                "migration move -id=%s -include-when='target.free > " +
            ...
            addCallback(
                  ...
                  new FutureCallback<Object>() {
                      ...
                      public void onSuccess(Object ignored) {
                          reply("Rebalancing jobs have been submitted to " + Joiner.on(", ")
                                .join(names) + ".");
                      }
