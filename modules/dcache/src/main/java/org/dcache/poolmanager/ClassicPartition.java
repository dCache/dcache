...
/**
 * Legacy partition that provided the classic dCache pool selection semantics. Now it only serves as
 * a base class for WassPartition containing old code that is expected to be replaced in the
 * future.
 */
public abstract class ClassicPartition extends Partition {
     ...
     * COSTFACTORS
     * <p>
     * spacecostfactor   double cpucostfactor     double
     * <p>
     * COSTCUTS
     * <p>
     * idle      double p2p       double alert     double halt      double slope     double fallback
     * double
     * <p>
     * OTHER max-copies       int fallback-onspace boolean
     * <p>
     * Options        |  Description ----------------------------------------------------------------------------
     * idle      |  below 'idle' : 'reduce duplicate' mode p2p       |  above : start pool to pool
     * mode |  If p2p value is a percent then p2p is dynamically |  assigned that percentile value
     * of pool performance costs alert      |  stop pool 2 pool mode, start stage only mode halt
     *  |  suspend system fallback    |  Allow fallback in Permission matrix on high load
     * fallback-onspace |  Allow fallback on write if out of free space error      |  How much the
     * performance cost of a pool may exceed |  limits before it rejects a request
     */
        ...
        if (costCut.endsWith("%")) {
            ...
            if (_costCut <= 0) {
                throw new IllegalArgumentException(
                      "Number " + _costCut + " is too small; must be > 0%");
            }
            if (_costCut >= 1) {
                throw new IllegalArgumentException(
                      "Number " + _costCut + " is too large; must be < 100%");
            }
        } else {
    ...
    public SelectedPool selectReadPool(CostModule cm,
         ...
         */
        double nextBest =
              (best.size() > 1) ? Math.max(best.get(1).getPerformanceCost(), mcc)
                    : Double.POSITIVE_INFINITY;

        return new SelectedPool(bestPool,
              PerformanceCostAssumption.of(_error, _panicCostCut, _fallbackCostCut, _costCut,
                    nextBest));
    }
    ...
    /**
     * Establish the costCut at the moment for the given set of parameters. If the costCut was
     * assigned a value from a string ending with a '%' then that percentile cost is used.
     *
    ...
    /**
     * Internal immutable helper class to hold precomputed performance and space cost.
     */
