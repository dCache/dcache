...
public class RepositoryInterpreter
    ...
    @Command(name = "rep set sticky", hint = "change sticky flags",
          description =
                "Changes the sticky flags on one or more replicas. Sticky flags prevent sweeper " +
    ...
    public class SetStickyCommand implements Callable<String> {
        ...
        @Option(name = "l", metaVar = "millis", category = "Sticky properties",
              usage =
                    "The lifetime in milliseconds from now. Once the lifetime expires, the sticky flag is "
                          +
        ...
        @Option(name = "rp", category = "Filter options", values = {"custodial", "replica",
              "output"},
        ...
        @Option(name = "cache", metaVar = "class", category = "Filter options",
              usage =
                    "Only change replicas with the given cache class. If set to the empty string, the condition "
                          +
        ...
        @Option(name = "all", category = "Filter options",
              usage =
                    "Allow using the command without any filter options and without a PNFS ID. This is a safe "
                          +
        ...
        public String call() throws CacheException, InterruptedException, CommandException {
            ...
            switch (state) {
                case "on":
                    expire = (lifetime != null) ? Math.max(0, System.currentTimeMillis() + lifetime)
                          : -1;
                    break;
                case "off":
                    expire = 0;
        ...
        protected boolean matches(PnfsId id) throws CacheException, InterruptedException {
            ...
            return (al == null || attributes.isDefined(FileAttribute.ACCESS_LATENCY)
                  && attributes.getAccessLatency().equals(al)) &&
                  (rp == null || attributes.isDefined(FileAttribute.RETENTION_POLICY)
                        && attributes.getRetentionPolicy().equals(rp)) &&
                  (storage == null
                        || attributes.isDefined(FileAttribute.STORAGECLASS) && Objects.equals(
                        attributes.getStorageClass(), storage)) &&
                  (cache == null
                        || attributes.isDefined(FileAttribute.CACHECLASS) && Objects.equals(
                        attributes.getCacheClass(), Strings.emptyToNull(cache)));
        }
    }
    ...
    @Command(name = "rep set broken", hint = "set replica broken",
          description =
                "Marks a replica as broken. Broken replicas are not served to clients. Such " +
    ...
    private static class Statistics {
        ...
        long[] toArray() {
            return new long[]{bytes, entries, preciousBytes, preciousEntries, stickyBytes,
                  stickyEntries, otherBytes, otherEntries};
        }
    }
