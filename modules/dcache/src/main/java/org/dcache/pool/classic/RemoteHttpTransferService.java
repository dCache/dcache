...
public class RemoteHttpTransferService extends SecureRemoteTransferService {
    ...
    protected MoverProtocol createMoverProtocol(ProtocolInfo info) throws Exception {
        ...
        if (info instanceof RemoteHttpsDataTransferProtocolInfo) {
            moverProtocol = new RemoteHttpsDataTransferProtocol(getCellEndpoint(), getValidator(),
                  secureRandom);
    ...
    private synchronized X509CertChainValidator getValidator() throws IOException {
        if (validator == null) {
            ...
            ValidatorParams validatorParams =
                  new ValidatorParams(
                        new RevocationParameters(getCrlCheckingMode(), ocspParameters),
                        ProxySupport.ALLOW);
            long updateInterval = getCertificateAuthorityUpdateIntervalUnit().toMillis(
                  getCertificateAuthorityUpdateInterval());
            validator = new OpensslCertChainValidator(getCertificateAuthorityPath(), true,
                  getNamespaceMode(), updateInterval, validatorParams,
                  false);
        }
        return validator;
    }
