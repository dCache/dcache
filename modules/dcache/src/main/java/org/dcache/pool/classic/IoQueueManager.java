...
public class IoQueueManager
    ...
    public MoverRequestScheduler getQueueByNameOrDefault(String queueName) {
        return (queueName == null) ? defaultQueue
              : queuesByName.getOrDefault(queueName, defaultQueue);
    }
    ...
    public int getOrCreateMover(String queueName, String doorUniqueId, MoverSupplier moverSupplier,
        ...
        return getQueueByNameOrDefault(queueName).getOrCreateMover(moverSupplier, doorUniqueId,
              priority);
    }
    ...
    public void printSetup(PrintWriter pw) {
        queues().forEach(
              q -> pw.println("mover queue create " + q.getName() + " -order=" + q.getOrder()));
        queues().forEach(q -> pw.println(
              "mover set max active -queue=" + q.getName() + " " + q.getMaxActiveJobs()));
        queues().stream()
    ...
    @Command(name = "mover queue create", hint = "create mover queue",
          description =
                "Creates a new mover queue. If the queue already exists, the command changes " +
    ...
    public class MoverQueueLsCommand implements Callable<Serializable> {
        ...
        public Serializable call() {
            ...
            if (verbose) {
                f = q -> q.getName() + " " + q.getActiveJobs() + " " + q.getMaxActiveJobs() + " "
                      + q.getQueueSize() + " " + q.getOrder();
            } else {
    ...
    public class JtmSetTimeoutCommand implements Callable<String> {
        ...
        public synchronized String call() throws CommandException {
            checkCommand(lastAccessed >= 0L, "invalid lastAccess value %d: must be >= 0",
                  lastAccessed);
    ...
    public class MoverLsCommand implements Callable<Serializable> {
        ...
        public Serializable call() throws CommandException {
            if (id != null) {
                return getQueueByJobId(id).orElseThrow(
                            () -> new CommandException("Id doesn't belong to any known scheduler."))
                      .getJobInfo(id)
                      .orElseThrow(() -> new CommandException("Job not found : Job-" + id));
            }
    ...
    public class MoverKillCommand implements Callable<String> {
        ...
        @Override
        public String call() throws CommandException {
            MoverRequestScheduler js = getQueueByJobId(id).orElseThrow(
                  () -> new CommandException("Id doesn't belong to any known scheduler."));
