...
/**
 * Encapsulates the tasks to be performed on the destination of a pool to pool transfer.
 * <p>
 * The companion will submit an HTTP download request to the source pool, wait for the reply and
 * then download the file through HTTP.
 * <p>
 * The code is driven by a state machine, Companion.sm. Most of the logic is encapsulated in the
 * state machine.
 */
class Companion {
     ...
     * @param checksumModule                Checksum module used to verify and compute checksums
    ...
    Companion(ScheduledExecutorService executor,
        ...
        _destinationPoolCellname = requireNonNull(destinationPoolCellname,
              "Destination pool name is unknown.");
        _destinationPoolCellDomainName = requireNonNull(destinationPoolCellDomainName,
        ...
        if (!_fileAttributes.isDefined(FileAttribute.PNFSID)) {
            throw new IllegalArgumentException(
                  "PNFSID is required, got " + _fileAttributes.getDefinedAttributes());
        }
    ...
    /**
     * Cancels the transfer. Returns true unless the transfer is already completed.
     */
    ...
    /**
     * Delivers a DoorTransferFinishedMessage. Normally send by the source pool.
     */
        ...
        try {
            ...
            HttpProtocolInfo protocolInfo =
                  new HttpProtocolInfo(
                        _sslContext != null ? PROTOCOL_INFO_SSL_NAME : PROTOCOL_INFO_NAME,
    ...
    synchronized void done() {
        if (_thread != null) {
            throw new IllegalStateException(
                  "Cannot close a companion while the transfer is in progress");
        }
