...
public interface Repository
    ...
    enum OpenFlags implements OpenOption {
         ...
         */
        NONBLOCK_SPACE_ALLOCATION,
    }

    /**
     * Loads the repository from the on disk state. Must be done exactly once before any other
     * operation can be performed. Depending on the implementation, some methods may not be
     * accesible until after the load method has been called.
     *
    ...
    /**
     * Loads the repository from the on disk state. Must be done exactly once after init was
     * called.
     *
     * @throws IllegalStateException if called multiple times
    ...
    /**
     * Creates entry in the repository. Returns a write handle for the entry. The write handle must
     * be explicitly closed. As long as the write handle is not closed, reads are not allowed on the
     * entry.
     * <p>
     * While the handle is open, the entry is in the transfer state. Once the handle is closed, the
     * entry is automatically moved to the target state, unless the handle is cancelled first.
     *
     ...
     * <p>
     * A read handle is returned which is used to access the file. The read handle must be
     * explicitly closed after use. While the read handle is open, it acts as a shared lock, which
     * prevents the entry from being deleted. Notice that an open read handle does not prevent state
     * changes.
     *
     ...
     * @param id the PNFS ID of the entry to open
     * @throws InterruptedException    if thread was interrupted
     * @throws FileNotInCacheException if file not found or in a state in which it cannot be opened
     * @throws CacheException          in case of other errors
     */
    ...
    /**
     * Sets the lifetime of a named sticky flag. If expiration time is -1, then the sticky flag
     * never expires. If is is 0, the flag expires immediately.
     *
     ...
     * @param overwrite replace existing flag when true, extend lifetime if false
     * @throws InterruptedException     if thread was interrupted
     * @throws FileNotInCacheException  when an entry with the given id is not found in the
     *                                  repository
     * @throws CacheException           in case of other errors
     * @throws IllegalArgumentException when <code>id</code> or
     *                                  <code>owner</code> are null or when <code>lifetime</code>
     *                                  is
     *                                  smaller than -1.
     */
    ...
    /**
     * Returns information about the size and space usage of the repository.
     *
    ...
    /**
     * Sets the state of an entry. Only the following transitions are allowed:
     *
     * <ul>
    ...
    /**
     * Returns a CompletableFuture that is asynchronously completed when repository {@code #load}
     * completes normally or completes exceptionally when load fails.
     *
