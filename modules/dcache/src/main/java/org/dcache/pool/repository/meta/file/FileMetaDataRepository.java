...
/**
 * This class wraps the old control- and SI-file- based metadata access method.
 */
public class FileMetaDataRepository
        ...
        if (!Files.exists(_metadir)) {
            if (readOnly) {
                throw new FileNotFoundException(
                      "No such directory and not allowed to create it: " + _metadir);
            }
    ...
    public Set<PnfsId> index(IndexOption... options) throws CacheException {
        try {
            ...
            if (indexOptions.contains(IndexOption.ALLOW_REPAIR)) {
                ...
                _log.info("Found {} orphaned meta data entries in {} in {}.",
    ...
    protected void deleteIfExists(Path path) throws DiskErrorCacheException {
        try {
            ...
            throw new DiskErrorCacheException(
                  "Failed to remove " + path + ": " + messageOrClassName(e), e);
        }
    }
