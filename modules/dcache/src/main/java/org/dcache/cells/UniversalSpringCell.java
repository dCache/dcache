 ...
 * <p>
 * This class aims at being a universal cell for dCache. It makes it possible to construct complex
 * cells from simpler cell components. The Spring Framework is used to manage the lifetime of the
 * cell components, as well as the wiring between components. We therfore borrow the term "bean"
 * from the Spring Framework and refer to cell components as beans.
 * <p>
 * Beans can get access to core cell functionality by implementing one or more of the following
 * interfaces: CellInfoProvider, CellCommunicationAware, ThreadFactoryAware, CellCommandListener,
 * and CellSetupAware. When instantiated through this class, those interfaces are detected and the
 * necessary wiring is performed automatically.
 */
public class UniversalSpringCell
    ...
    private static final Class<?>[] TERMINAL_TYPES = new Class<?>[]{Class.class,
          ApplicationContext.class};
    private static final Class<?>[] HIDDEN_TYPES = new Class<?>[]{ApplicationContext.class,
          AutoCloseable.class};
    ...
    /**
     * Spring application context. All beans are created through this context.
     */
    private ConfigurableApplicationContext _context;

    /**
     * Registered info providers mapped to their bean names. Sorted to maintain consistent
     * ordering.
     */
    ...
    /**
     * List of registered setup providers. Sorted to maintain consistent ordering.
     */
    private final Map<String, CellSetupProvider> _setupProviders =
          new TreeMap<>();

    /**
     * List of registered life cycle aware beans. Sorted to maintain consistent ordering.
     */
    ...
    private void waitForFiles()
        ...
        if (s != null && !s.trim().isEmpty()) {
            ...
            while ((missing = firstMissing(files)) != null) {
                LOGGER.warn("File missing: {}; sleeping {} seconds", missing,
                      WAIT_FOR_FILE_SLEEP / 1000);
    ...
    /**
     * Returns the singleton bean with a given name. Returns null if such a bean does not exist.
     */
    ...
    /**
     * Collects information about the cell and returns these in a CellInfo object. Information is
     * collected from the CellAdapter base class and from all beans implementing CellInfoProvider.
     */
    @Override
    ...
    /**
     * If given a simple bean name, returns that bean (equivalent to calling getBean). If given a
     * compound name using the syntax of BeanWrapper, then the respective property value is
     * returned. E.g., getBeanProperty("foo") returns the bean named "foo", whereas
     * getBeanProperty("foo.bar") returns the value of the "bar" property of bean "foo".
     */
            ...
            if (o != null) {
                ...
                for (PropertyDescriptor p : bean.getPropertyDescriptors()) {
                    ...
                    if (bean.isReadableProperty(property)) {
                        ...
                        try {
                            s.append(org.dcache.util.Strings.toString(
                                  bean.getPropertyValue(property)));
    ...
    public class BeanPropertyCommand implements Callable<String> {
        ...
        public String call() throws InvalidPropertyException {
            Object o = getBeanProperty(name);
            return (o != null) ? org.dcache.util.Strings.toMultilineString(o)
    ...
    public class BeanMessagesCommand implements Callable<String> {
        ...
        public String call() {
            if (name == null) {
                ...
                f.format(format, "-------", "---------");
                for (Map.Entry<Class<? extends Serializable>, Collection<String>> e : classToNameMap.asMap()
                      .entrySet()) {
                    f.format(format,
    ...
    public BeanQueryMessage messageArrived(BeanQuerySinglePropertyMessage message)
        ...
        String name = a[0];
        Object o = null;
        if (_context != null && _context.isSingleton(name) && _context.containsBeanDefinition(
              name)) {
    ...
    /**
     * Registers an info provider. Info providers contribute to the result of the
    ...
    /**
     * Add a message receiver. Message receiver receive messages via message handlers (see
    ...
    /**
     * Registers a setup provider. Setup providers contribute to the result of the <code>save</code>
     * method.
     */
    ...
    /**
     * Registers a life cycle aware bean. Life cycle aware beans are notified about cell start and
     * stop events.
     */
    ...
    /**
     * Part of the BeanPostProcessor implementation. Recognizes beans implementing
     * CellCommandListener, CellInfoProvider, CellCommunicationAware, CellSetupProvider and
        ...
        public boolean isReadableProperty(String propertyName) {
            if (isAllowedName(propertyName)) {
                try {
                    ...
                    if (pd.getReadMethod() != null && !pd.isHidden() && isAllowedType(
                          pd.getPropertyType())) {
                        return true;
                    }
                } catch (InvalidPropertyException e) {
    ...
    private interface SetupManager {
        ...
        /**
         * Called when a setup command is issued. The implementation should execute the given
         * command and optionally synchronize it with external representations.
         */
        Serializable execute(CommandExecutor command, Args args) throws CommandException;

        /**
         * Called when the admin issues the reload command. Implementations should parse the given
         * file and update the configuration accordingly.
         */
    ...
    private class InMemorySetupManager implements SetupManager {
        ...
        @Override
        public synchronized Serializable execute(CommandExecutor command, Args args)
              throws CommandException {
    ...
    /**
     * Setup manager that uses an local file as an initial configuration source and afterwards keeps
     * the current setup in sync with a zookeeper node.
     */
    private class ZooKeeperSetupManager implements NodeCacheListener, SetupManager {
        ...
        public synchronized void start() throws CommandException {
            ...
            if (currentData == null) {
                throw new CommandPanicException("Setup reload failed because " + _node
                      + " in ZooKeeper disappeared. Service must be restarted.");
            }
            apply(currentData);
        }
        ...
        public synchronized Serializable execute(CommandExecutor command, Args args)
            ...
            if (_current != null) {
                try {
                    ...
                    _current = _curator.setData().withVersion(_current.getVersion())
                          .forPath(_node, data);
                    ...
                    rollback(e);
                    throw new CommandPanicException(
                          "Setup command failed because " + _node
                                + " in ZooKeeper disappeared. Service must be restarted.", e);
        ...
        public void load(File file) throws CommandException {
            try {
                ...
                throw new CommandThrowableException(
                      "Setup reload fail due to ZooKeeper failure: " + e.getMessage(), e);
            } catch (InterruptedException e) {
        ...
        public synchronized void nodeChanged() throws Exception {
            ...
            if (newData == null) {
                LOGGER.error("Setup node {} in ZooKeeper disappeared. Service must be restarted.",
                      _node);
                kill();
        ...
        private void rollback(Exception cause) throws CommandPanicException {
            try {
                ...
                if (currentData == null) {
                    kill();
                    throw new CommandPanicException(
                          "Setup node " + _node + " in ZooKeeper disappeared.");
                }
                apply(currentData);
            } catch (CommandException e) {
                e.addSuppressed(cause);
                throw new CommandPanicException(
                      "Saving setup to ZooKeeper failed (" + cause.getMessage()
                            + "); however the rollback failed too (" + e.getMessage()
                            + "). Service must be restarted.", e);
            }
        }
