...
public class TransferCollector {
    ...
    public ListenableFuture<Collection<LoginManagerChildrenInfo>> collectLoginManagerInfo(
          Set<CellPath> loginManagers) {
        return transform(
    ...
    public static Set<CellPath> getLoginManagers(Collection<LoginBrokerInfo> loginBrokerInfos) {
        return loginBrokerInfos.stream().map(d -> new CellPath(d.getCellName(), d.getDomainName()))
              .collect(toSet());
    }
    ...
    public static Set<CellPath> getDoors(Collection<LoginManagerChildrenInfo> loginManagerInfos) {
        return loginManagerInfos.stream()
              .flatMap(
                    i -> i.getChildren().stream().map(c -> new CellPath(c, i.getCellDomainName())))
              .collect(toSet());
    }

    public static List<Transfer> getTransfers(Collection<IoDoorInfo> doors,
    ...
    private static Map<String, IoJobInfo> createIndex(Collection<IoJobInfo> movers) {
        ...
        for (IoJobInfo info : Ordering.natural().onResultOf(IoJobInfo::getJobId)
              .sortedCopy(movers)) {
    ...
    private static Stream<Transfer> getTransfers(IoDoorInfo door, Map<String, IoJobInfo> movers) {
        return door.getIoDoorEntries().stream()
              .map(e -> new Transfer(door, e, movers.get(getKey(door, e))));
    }
    ...
    private <T> ListenableFuture<List<T>> query(Collection<CellPath> cells, String query,
          Class<T> returnType,
