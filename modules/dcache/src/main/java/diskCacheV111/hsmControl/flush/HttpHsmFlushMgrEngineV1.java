...
public class HttpHsmFlushMgrEngineV1 implements HttpResponseEngine, CellMessageSender {
    ...
    public void queryUrl(HttpRequest request) throws HttpException {
        ...
        try {
            ...
            if ((urlItems.length > 1) && (urlItems[1].equals("css"))) {
                ...
                CellStub flushManager = new CellStub(_endpoint, new CellPath(flushManagerName), 20,
                      SECONDS);
                ...
                if (!flushManagerName.equals("*")) {
                    ...
                    try {
                        ...
                        pw.println("<center><h2>Flush Manager " + flushManagerName
                              + " seems not to be present</h2>");
                    }
                }
            }
        } catch (Exception ee) {
            ...
            pw.println("<address>Last Modified : " + (new Date())
                  + " $Id: HttpHsmFlushMgrEngineV1.java,v 1.3 2006-05-20 10:40:02 patrick Exp $</address>");
            pw.println("</body>");
            pw.println("</html>");
        }
    }
    ...
    private void printCellInfo(PrintWriter pw, CellStub flushManager) throws Exception {
        try {
            FlushControlCellInfo info = flushManager.sendAndWait("xgetcellinfo",
                  FlushControlCellInfo.class);
    ...
    private void prepareCellInfo(PrintWriter pw, String flushManagerName,
          FlushControlCellInfo info) {
    ...
    private void printFlushManagerList(PrintWriter pw, CellStub flushManager, Map<?, ?> options)
          throws Exception {
        try {
    ...
    private void doActionsIfNecessary(CellStub flushManager, Map<?, ?> options,
          StringBuffer output) {
    ...
    private void preparePoolList(PrintWriter pw, String flushManagerName, Map<?, ?> options,
          List<HsmFlushControlCore.PoolDetails> list) {
    ...
    private void printFlushingPools(PrintWriter pw, String flushManagerName, Map<?, ?> options,
          List<HttpFlushManagerHelper.PoolEntry> pools) {
        ...
        pw.println("</tr>");
        int row = 0;
        for (Iterator<HttpFlushManagerHelper.PoolEntry> it = pools.iterator(); it.hasNext();
              row++) {
    ...
    private void printFlushingStorageClasses(PrintWriter pw, String flushManagerName,
          Map<?, ?> options, List<HttpFlushManagerHelper.FlushEntry> pools) {
        ...
        pw.println("</tr>");
        int row = 0;
        for (Iterator<HttpFlushManagerHelper.FlushEntry> it = pools.iterator(); it.hasNext();
              row++) {
    ...
    private void printDirectory(PrintWriter pw, int position) {
        ...
        pw.println(
              "<br><center><h2 class=\"m-table\">Flush Managers</h2><table class=\"m-table\">");
    ...
    private void printInternalCssFile(PrintWriter pw) {
        ...
        pw.println(
              "tr.s-table-a { background-color:#bebebe ; text-align:center ; font-size:16 ; }");
        pw.println(
              "tr.s-table-b { background-color:#efefef ; text-align:center ; font-size:16 ; }");
