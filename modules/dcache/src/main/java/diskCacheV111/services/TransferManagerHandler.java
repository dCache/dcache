...
public class TransferManagerHandler extends AbstractMessageCallback<Message> {
    ...
    private static final Map<Integer, String> STATE_DESCRIPTION =
          ImmutableMap.<Integer, String>builder()
                ...
                .put(RECEIVED_PNFS_INFO_STATE, "File metadata received")
                .put(WAITING_FOR_CREATED_FILE_INFO_STATE, "Querying created file metadata")
                .put(RECEIVED_CREATED_FILE_INFO_STATE, "Created file metadata received")
                ...
                .put(WAITING_FIRST_POOL_REPLY_STATE, "Requesting mover")
                .put(RECEIVED_FIRST_POOL_REPLY_STATE, "Mover created")
                ...
                .put(RECEIVED_PNFS_ENTRY_DELETE, "File deleted")
                .put(WAITING_FOR_PNFS_CHECK_BEFORE_DELETE_STATE, "Checking before deletion")
                .put(RECEIVED_PNFS_CHECK_BEFORE_DELETE_STATE, "OK to delete confirmed")
                .put(SENT_ERROR_REPLY_STATE, "Failed")
    ...
    public void success(Message message) {
        try {
            ...
            log.error("Bug detected in transfermanager, please report this to <support@dCache.org>",
                  e);
                ...
                switch (rc) {
                    ...
                    default:
                        if (numberOfMoverStartRetries++ < MAXIMUM_MOVER_START_ATTEMPTS) {
                            log.debug(
                                  "Pool {} reported rc={}; scheduling another attempt to start mover",
                                  pool.getAddress(), rc);
                            executor.execute(() -> {
                                try {
                                    Thread.sleep(1_000);
    ...
    public void createEntryResponseArrived(PnfsCreateEntryMessage create) {
        ...
        if (create.getFileAttributes().isDefined(STORAGEINFO)
              && create.getFileAttributes().getStorageInfo().getKey("path") != null) {
    ...
    public void storageInfoArrived(PnfsGetFileAttributes msg) {
        ...
        log.debug("storageInfoArrived(uid={} gid={} pnfsid={} fileAttributes={}", info.getUid(),
              info.getGid(),
    ...
    public void poolFirstReplyArrived(PoolIoFileMessage poolMessage) {
        ...
        log.debug("Pool {} will deliver file {} mover id is {}", pool, pnfsId,
              poolMessage.getMoverId());
    ...
    public void poolDoorMessageArrived(DoorTransferFinishedMessage doorMessage) {
        log.debug("poolDoorMessageArrived, doorMessage.getReturnCode()={}",
    ...
    public synchronized String toString(boolean isLongFormat) {
        ...
        String siPath =
              fileAttributes == null ? null : fileAttributes.getStorageInfo().getKey("path");
        ...
        if (isLongFormat) {
            ...
            sb.append("    User: ").append(Subjects.getDisplayName(transferRequest.getSubject()))
                  .append('\n');
            ...
            if (fileAttributes != null) {
                if (fileAttributes.isDefined(ACCESS_LATENCY)) {
                    sb.append("    Access latency: ").append(fileAttributes.getAccessLatency())
                          .append('\n');
                }
                if (fileAttributes.isDefined(RETENTION_POLICY)) {
                    sb.append("    Retention policy: ").append(fileAttributes.getRetentionPolicy())
                          .append('\n');
                }
                if (fileAttributes.isDefined(STORAGECLASS)) {
                    sb.append("    Storage class: ").append(fileAttributes.getStorageClass())
                          .append('\n');
                }
