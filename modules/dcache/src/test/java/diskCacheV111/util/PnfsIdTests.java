 ...
 * <p>
 * The tests can be grouped into three classes of test: those that test a "simple" (without a
 * domain) PNFS ID , those that test a PNFS ID with domain and those that test a Chimera ID.
 * <p>
 * Features, such as star-expanding and support for truncated PNFS IDs are also tested, but only
 * that they are equal to the corresponding simple PNFS ID.
 */
