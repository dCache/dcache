...
public class EventNotifierTest {
    ...
    private void verifyCellStubNeverSendAndWait()
        ...
        verify(cellStub, never()).sendAndWait((CellPath) any(), (Serializable) any(),
              (Class<Message>) any());
        verify(cellStub, never()).sendAndWait((CellPath) any(), (Message) any(), anyInt());
        verify(cellStub, never()).sendAndWait((Serializable) any(), (Class<Message>) any(),
              anyInt());
        verify(cellStub, never()).sendAndWait((CellPath) any(), (Serializable) any(),
              (Class<Message>) any(), anyInt());
    }
    ...
    private void verifyCellStubNeverSend() {
        ...
        verify(cellStub, never()).send((CellPath) any(), (Serializable) any(),
              (Class<Message>) any());
    ...
    private void verifyNoOutboundMessage()
          throws CacheException, InterruptedException, NoRouteToCellException {
    ...
    private List<NotificationMessage> sentNotificationMessages()
          throws InterruptedException, CacheException, NoRouteToCellException {
        ArgumentCaptor<NotificationMessage> argument = ArgumentCaptor.forClass(
    ...
    public void shouldStartSenderTaskWhenSubscribedEventFires() throws Exception {
        ...
        givenZooKeeperEvent(CHILD_ADDED, "/dcache/inotify/cell@domain", target,
    ...
    public void shouldSendEventWhenSubscribedEventFires() throws Exception {
        ...
        InotifyEvent expected = new InotifyEvent(EventType.IN_MOVE_SELF, target, null, null,
              REGULAR);
    ...
    private void givenZooKeeperEvent(PathChildrenCacheEvent.Type type, String path, PnfsId id,
