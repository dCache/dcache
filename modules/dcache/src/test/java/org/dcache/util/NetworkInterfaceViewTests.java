 ...
 * <p>
 * We are somewhat limited in what we can test as NetworkInterface is a final class that doesn't
 * implement any interface and there are no static methods for creating NetworkInterface objects
 * other than those for returning information about what is available.
 * <p>
 * These tests will use all the interfaces of the test-machine from which we can create a
 * NetworkInterfaceView snapshot as the fixture. Any interface that we cannot create a snapshot of
 * is silently ignored. See {@link NetworkInterfaceView} for reasons why creating a snapshot might
 * fail.
 * <p>
 * These tests are bypassed (they are not run but without reporting a failure) if we can't enumerate
 * the available interfaces or if we can't take a snapshot of at least one interface.
 */
public class NetworkInterfaceViewTests {
    ...
    public void testGetInterfaceAddresses() {
        for (InterfaceAndViewPair pair : _interfaces) {
            ...
            for (int i = 0; i < sourceAddresses.size(); i++) {
                ...
                NetworkInterfaceView.InterfaceAddressView expected = new NetworkInterfaceView.InterfaceAddressView(
                      source);
    ...
    /**
     * A simple data class to hold a network interface and the corresponding NetworkInterfaceView
     * snapshot.
     */
