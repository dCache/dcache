...
public class PinManagerTests {
    ...
    final static diskCacheV111.vehicles.Pool POOL1 = new diskCacheV111.vehicles.Pool("pool1",
          new CellAddressCore("pool1"), Assumptions.none());
...
class TestDao implements PinDao {
    ...
    public List<Pin> get(PinCriterion criterion) {
        return _pins.values().stream().filter(((TestCriterion) criterion)::matches)
              .collect(toList());
    }

    @Override
    public List<Pin> get(PinCriterion criterion, int limit) {
        return _pins.values().stream().filter(((TestCriterion) criterion)::matches).limit(limit)
              .collect(toList());
    }

    @Override
    public Pin get(UniquePinCriterion criterion) {
        return _pins.values().stream().filter(((TestCriterion) criterion)::matches).findFirst()
              .orElse(null);
    }
    ...
    public void foreach(PinCriterion criterion, InterruptibleConsumer<Pin> f, int limit)
        ...
        if (limit < 0) {
            throw new IllegalArgumentException(
                  "The chunked pin iteration must not be called with a negative limit value.");
        }
        doChunkedForeach(criterion, f, limit);
    }

    private void doChunkedForeach(PinCriterion criterion, InterruptibleConsumer<Pin> f, int limit)
          throws InterruptedException {
    ...
    private static class TestUpdate implements PinUpdate {
        ...
        public PinUpdate expirationTime(Date date) {
            return add(
                  p -> new Pin(p.getPinId(), p.getPnfsId(), p.getRequestId(), p.getCreationTime(),
                        date,
...
class TestPoolManagerStub extends PoolManagerStub implements CellMessageReceiver {
    ...
    public TestPoolManagerStub(CellAddressCore address) {
        ...
        setHandler(new PoolManagerHandler() {
            ...
            @Override
            public <T extends PoolManagerMessage> ListenableFuture<T> sendAsync(
                  CellEndpoint endpoint, T msg, long timeout) {
                return Futures.immediateFuture(msg);
            }
