...
public class MultiHSMTest extends TestCase {
    ...
    public void testStore() throws Exception {
        ...
        StorageInfo storageInfo = new GenericStorageInfo();
        assertEquals("SotrageInfo without URL shoul not declare itself as stored", false,
              storageInfo.isStored());

        // legacy case

        storageInfo.setIsStored(true);
        assertEquals("SotrageInfo after  setIsStored(true) shold shoul declare itself as stored",
              true, storageInfo.isStored());

        // legacy case
        storageInfo.setIsStored(false);
        assertEquals(
              "SotrageInfo after  setIsStored(false) shold shoul not declare itself as stored",
              false, storageInfo.isStored());

        String osmLocationMain = "osm://desy-main/?store=h1&bfid=1234";
        storageInfo.addLocation(new URI(osmLocationMain));
        assertEquals("SotrageInfo with URL shoul declare itself as stored", true,
              storageInfo.isStored());
    }
