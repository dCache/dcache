#!/bin/sh
#
# $Id: install.sh,v 1.5.2.6 2007-10-10 12:37:04 omsynge Exp $
#
...
#
# check java:
#     jdk >= 1.4 , ( javac needed by tomcat/SRM )
#
...
${java} -version 2>&1 | grep version | egrep "1\.[456]\." -q
if [ $? -ne 0 ]; then
	echo "[ERROR] java variable in ${DCACHE_BASE_DIR}/config/dCacheSetup do not point to java version 1.4.x or 1.5.x"
	exit 6
fi
